...
# Wonjae Kim\*1† Bokyung Son\*1 Ildoo Kim²
...
<sup>\*</sup>Equal contribution †Current affiliation: NAVER AI Lab, Seongnam, Gyeonggi, Republic of Korea. ¹Kakao Enterprise, Seongnam, Gyeonggi, Republic of Korea ²Kakao Brain, Seongnam, Gyeonggi, Republic of Korea. Correspondence to: Wonjae Kim <wonjae.kim@navercorp.com>.

<sup>&</sup>lt;sup>1</sup>While some works employ additional objectives and data structures, these two objectives apply to almost every VLP model.
...
<sup>&</sup>lt;sup>2</sup>Bugliarello et al. (2020) showed that a controlled setup bridges the performance gap of various region-feature-based VLP models.
...
$$\bar{t} = [t_{\text{class}}; t_1 T; \dots; t_L T] + T^{\text{pos}}$$
(1)

$$\bar{v} = [v_{\text{class}}; v_1 V; \dots; v_N V] + V^{\text{pos}}$$
(2)

$$z^0 = [\bar{t} + t^{\text{type}}; \bar{v} + v^{\text{type}}] \tag{3}$$

$$\hat{z}^d = MSA(LN(z^{d-1})) + z^{d-1}, \qquad d = 1...D$$
 (4)

$$z^{d} = \text{MLP}(\text{LN}(\hat{z}^{d})) + \hat{z}^{d}, \qquad d = 1 \dots D \quad (5)$$

$$p = \tanh(z_0^D W_{\text{pool}}) \tag{6}$$
...
<sup>&</sup>lt;sup>3</sup>Parameters for R50 is 25M, R101 is 44M, and X152 is 60M.

<sup>&</sup>lt;sup>4</sup>We also experimented with initializing the layers from BERT weights and using the pre-trained patch projection from ViT, but it did not work.
...
The input image  $I \in \mathbb{R}^{C \times H \times W}$  is sliced into patches and flattened to  $v \in \mathbb{R}^{N \times (P^2 \cdot C)}$  where (P,P) is the patch resolution and  $N = HW/P^2$ . Followed by linear projection  $V \in \mathbb{R}^{(P^2 \cdot C) \times H}$  and position embedding  $V^{\mathrm{pos}} \in \mathbb{R}^{(N+1) \times H}$ , v is embedded into  $\bar{v} \in \mathbb{R}^{N \times H}$ .

The text and image embeddings are summed with their corresponding modal-type embedding vectors  $t^{\mathrm{type}}, v^{\mathrm{type}} \in \mathbb{R}^H$ , then are concatenated into a combined sequence  $z^0$ . The contextualized vector z is iteratively updated through D-depth transformer layers up until the final contextualized sequence  $z^D$ . p is a pooled representation of the whole multimodal input, and is obtained by applying linear projection  $W_{\mathrm{pool}} \in \mathbb{R}^{H \times H}$  and hyperbolic tangent upon the first index of sequence  $z^D$ .

For all experiments, we use weights from ViT-B/32 pretrained on ImageNet, hence the name ViLT-B/32.<sup>5</sup> Hidden size H is 768, layer depth D is 12, patch size P is 32, MLP size is 3,072, and the number of attention heads is 12.
...
**Masked Language Modeling.** This objective is to predict the ground truth labels of masked text tokens  $t_{\rm masked}$  from its contextualized vector  $z_{\rm masked}^D|_t$ . Following the heuristics of Devlin et al. (2019), we randomly mask t with the probability of 0.15.

We use a two-layer MLP MLM head that inputs  $z_{\mathrm{masked}}^D|_t$  and outputs logits over vocabulary, just as the MLM objective of BERT. The MLM loss is then computed as the negative log-likelihood loss for the masked tokens.
...
<sup>&</sup>lt;sup>5</sup>ViT-B/32 is pre-trained with ImageNet-21K and fine-tuned on ImageNet-1K for image classification. We expect that weights pre-trained on larger datasets (e.g., JFT-300M) would yield better performance.
...
<sup>&</sup>lt;sup>6</sup>VQA score is calculated by comparing the inferred answer to 10 ground-truth answers: see https://visualqa.org/evaluation.html for details.
...
| Visual<br>Embed | Model                                 | Time                 | FI           |              | Zero-Shot Tex<br>Flickr30k (1K) |              | xt Retrieval<br>MSCOCO (5K) |              | Zero-Shot Image Retrieval<br>Flickr30k (1K) MSCOCO (5K) |                      |                      |              | (5K)         |              |
|-----------------|---------------------------------------|----------------------|--------------|--------------|---------------------------------|--------------|-----------------------------|--------------|---------------------------------------------------------|----------------------|----------------------|--------------|--------------|--------------|
|                 |                                       | (ms)                 | R@1          | R@5          | R@10                            | R@1          | R@5                         | R@10         | R@1                                                     | R@5                  | R@10                 | R@1          | R@5          | R@10         |
| Region          | ViLBERT<br>Unicoder-VL<br>UNITER-Base | ~900<br>~925<br>~900 | 64.3<br>80.7 | 85.8<br>95.7 | 92.3<br>98.0                    | -            | -                           | -<br>-<br>-  | 31.9<br>48.4<br>66.2                                    | 61.1<br>76.0<br>88.4 | 72.8<br>85.2<br>92.9 | -            | -            | -            |
|                 | ImageBERT†                            | ~925                 | 70.7         | 90.2         | 94.0                            | 44.0         | 71.2                        | 80.4         | 54.3                                                    | 79.6                 | 87.5                 | 32.3         | 59.0         | 70.2         |
| Linear          | ViLT-B/32<br>ViLT-B/32⊕               | ~15<br>~15           | 69.7<br>73.2 | 91.0<br>93.6 | 96.0<br>96.5                    | 53.4<br>56.5 | 80.7<br>82.6                | 88.8<br>89.6 | 51.3<br>55.0                                            | 79.9<br>82.5         | 87.9<br>89.8         | 37.3<br>40.4 | 67.4<br>70.0 | 79.0<br>81.1 |
...
| Visual |                          | Time | Text Retrieval |                  |             |      |                | Image Retrieval |      |                  |             |      |                |              |
|--------|--------------------------|------|----------------|------------------|-------------|------|----------------|-----------------|------|------------------|-------------|------|----------------|--------------|
| Embed  | Model                    | (ms) | R@1            | ickr30k (<br>R@5 | 1K)<br>R@10 | R@1  | SCOCO (<br>R@5 | (5K)<br>R@10    | R@1  | ickr30k (<br>R@5 | 1K)<br>R@10 | R@1  | SCOCO (<br>R@5 | (5K)<br>R@10 |
|        |                          |      | K@1            | K@3              | K@10        | K@1  | K@3            | K@10            | K@1  | K@3              | K@10        | K@1  | K@3            | K@10         |
|        | w/o VLP SOTA             | ~900 | 67.4           | 90.3             | 95.8        | 50.4 | 82.2           | 90.0            | 48.6 | 77.7             | 85.2        | 38.6 | 69.3           | 80.4         |
|        | ViLBERT-Base             | ~920 | -              | -                | -           | -    | -              | -               | 58.2 | 84.9             | 91.5        | -    | -              | -            |
| Region | Unicoder-VL              | ~925 | 86.2           | 96.3             | 99.0        | 62.3 | 87.1           | 92.8            | 71.5 | 91.2             | 95.2        | 48.4 | 76.7           | 85.9         |
...
| Linear | ViLT-B/32@               | ~15  | 83.7           | 97.2             | 98.1        | 62.9 | 87.1           | 92.7            | 62.2 | 87.6             | 93.2        | 42.6 | 72.8           | 83.4         |
...
We report the zero shot retrieval results in Table 3 and finetuned results in Table 4. At zero-shot retrieval, ViLT-B/32 performs better in general than ImageBERT despite Image-BERT's pre-training on a larger (14M) dataset. At fine-tuned retrieval, recalls for ViLT-B/32 are higher by a large margin than the second fastest model (Pixel-BERT-R50).
...
<sup>&</sup>lt;sup>7</sup>UNITER proposed three downstream head setups: pair, triplet, and pair-biattn.

<sup>&</sup>lt;sup>8</sup>R@K corresponds to whether the ground truth is included among top K results from the validation set.

| <i>Table 5.</i> Ablation study of ViLT-B/32. | w denotes whether whole word masking is u      | used for pre-training. |
|----------------------------------------------|------------------------------------------------|------------------------|
| objective is used for pre-training. (a) de   | enotes whether RandAugment is used during fine | e-tuning.              |

| Training | A | Ablatio | n   | VQAv2            | NL               | VR2              |               | R@1 (1K)      | MSCOCO R@1 (5K) |               |  |
|----------|---|---------|-----|------------------|------------------|------------------|---------------|---------------|-----------------|---------------|--|
...
| 100K     | O | X       | 0   | $70.85 \pm 0.13$ | $74.91 \pm 0.29$ | $75.57 \pm 0.61$ | 83.69 (69.73) | 62.22 (51.28) | 62.88 (53.40)   | 42.62 (37.26) |  |
...
|--------|--------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------|-----------------------------------------------------------------------|--------------------------------------------------------------|
| Embed  |                                                                                                                                                  | (M)                                                         | (G)                                                                   | (ms)                                                         |
| Region | ViLBERT36+36<br>VisualBERT36+128<br>LXMERT36+20<br>UNITER-Base36+60<br>OSCAR-Base50+35<br>VinVL-Base50+35<br>Unicoder-VL100+?<br>ImageBERT100+44 | 274.3<br>170.3<br>239.8<br>154.7<br>154.7<br>157.3<br>170.3 | 958.1<br>425.0<br>952.0<br>949.9<br>956.4<br>1023.3<br>419.7<br>420.6 | ~900<br>~925<br>~900<br>~900<br>~900<br>~650<br>~925<br>~925 |
...
| Visual<br>Embed | Model                                                                             | CNN<br>Backbone                                              | RoI<br>Head                                     | NMS                                          | Trans.<br>Layers                               |
|-----------------|-----------------------------------------------------------------------------------|--------------------------------------------------------------|-------------------------------------------------|----------------------------------------------|------------------------------------------------|
| Region          | ViLBERT VisualBERT LXMERT UNITER-Base OSCAR-Base VinVL-Base Unicoder-VL ImageBERT | R101<br>X152<br>R101<br>R101<br>R101<br>X152<br>X152<br>X152 | C4<br>FPN<br>C4<br>C4<br>C4<br>C4<br>FPN<br>FPN | PC<br>PC<br>PC<br>PC<br>PC<br>CA<br>PC<br>PC | ~15<br>12<br>~12<br>12<br>12<br>12<br>12<br>12 |
| Grid            | Pixel-BERT-X152<br>Pixel-BERT-R50                                                 | X152<br>R50                                                  | -<br>-                                          | -                                            | 12<br>12                                       |
| Linear          | ViLT-B/32                                                                         | -                                                            | -                                               | -                                            | 12                                             |
...
<sup>&</sup>lt;sup>9</sup>FLOPs and time are neglectable because the operation is an embedding lookup. The 30K embedding dictionary used by bert-base-uncased has 23.47 M parameters
...
- Anderson, P., He, X., Buehler, C., Teney, D., Johnson, M., Gould, S., and Zhang, L. Bottom-up and top-down attention for image captioning and visual question answering. In *Proceedings of the IEEE conference on computer vi*sion and pattern recognition, pp. 6077–6086, 2018.
