...
Matt Deitke $^{\dagger\psi}$ , Ruoshi Liu $^{\gamma}$ , Matthew Wallingford $^{\psi}$ , Huong Ngo $^{\psi}$ , Oscar Michel $^{\dagger}$ , Aditya Kusupati $^{\psi}$ , Alan Fan $^{\psi}$ , Christian Laforte $^{\sigma}$ , Vikram Voleti $^{\sigma}$ , Samir Yitzhak Gadre $^{\gamma}$ , Eli VanderBilt $^{\dagger}$ , Aniruddha Kembhavi $^{\dagger\psi}$ , Carl Vondrick $^{\gamma}$ , Georgia Gkioxari $^{\delta}$ , Kiana Ehsani $^{\dagger}$ , \*Ludwig Schmidt $^{\dagger\psi\ell}$ , \*Ali Farhadi $^{\psi}$ 

 $^\dagger Allen$  Institute for AI  $^\psi U$ niversity of Washington, Seattle  $^\gamma C$ olumbia University  $^\sigma S$ tability AI  $^\delta C$ alifornia Institute of Technology  $^\ell L$ AION  $^* E$ qual Senior Contribution
...
| Figure 2: t-SNE projection of CLIP L/14 embeddings on a subset of  |
|--------------------------------------------------------------------|
| rendered objects. Compared to Objaverse 1.0 (orange), Objaverse-XL |
| more densely captures the distribution of 3D assets.               |
...
Figure 6: **Novel view synthesis at scale. Left**: PixelNeRF [64] trained on varying scales of data and evaluated on a held-out subset of Objavserse-XL. **Right**: Zero123 [36] trained on varying scales of data and evaluated on a zero-shot dataset. Note that the 800K datapoint is Zero123 and the 10M datapoint is Zero123-XL. The synthesis quality consistently improves with scale. LPIPS is scaled up 10 times for visualization.
...
|-------------------------|--------------------------------|--------------------------|
| Base<br>w/ Objaverse-XL | $15.32$ <b>17.53</b> $\pm .37$ | 22.71 <b>24.22</b> ± .55 |
...
- [32] J. J. Lim, H. Pirsiavash, and A. Torralba. Parsing ikea objects: Fine pose estimation. In *Proceedings of the IEEE international conference on computer vision*, pages 2992–2999, 2013.
...
- [60] Q. Wang, Z. Wang, K. Genova, P. P. Srinivasan, H. Zhou, J. T. Barron, R. Martin-Brualla, N. Snavely, and T. Funkhouser. Ibrnet: Learning multi-view image-based rendering. In Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition, pages 4690–4699, 2021.
...
Is it possible to identify individuals (i.e., one or more natural persons), either directly or indirectly (i.e., in combination with other data) from the dataset? If so, please describe how. If a scanned person is included in the dataset, it may be possible to visually identify them or identify them if their name is included as part of the metadata.
...
Was the "raw" data saved in addition to the preprocessed/cleaned/labeled data (e.g., to support unanticipated future uses)? If so, please provide a link or other access point to the "raw" data. s The data that is downloaded contains the raw data and does not modify the individual files.
...
Has the dataset been used for any tasks already? If so, please provide a description. Yes, please see Section 4 of the paper.
...
Are there tasks for which the dataset should not be used? If so, please provide a description. New tasks must make sure to follow the license of the dataset and the license of the individual objects distributed as part of the dataset.

Any other comments? No.

### C.6 Distribution

Will the dataset be distributed to third parties outside of the entity (e.g., company, institution, organization) on behalf of which the dataset was created? If so, please provide a description. Yes. The dataset will be made public.
...
How can the owner/curator/manager of the dataset be contacted (e.g., email address)? Please contact mattd@allenai.org.
...
| Category | Description                      | <b>Aesthetic Score Cutoff</b> | Percentage of Dataset |
|----------|----------------------------------|-------------------------------|-----------------------|
