...
correctly predict the missing word in the following sentence: "The \_\_\_ is the currency of the United Kingdom" (answer: "pound").
...
<sup>\*</sup>Equal contribution <sup>1</sup>Google Research. Correspondence to: Kelvin Guu <kguu@google.com>, Kenton Lee <kentonl@google.com>, Zora Tung <gatoatigrado@google.com>, Panupong Pasupat ppasupat@google.com>, Ming-Wei Chang <mingweichang@google.com>.
...
<sup>&</sup>lt;sup>1</sup>We use the term "document" loosely to refer to a passage from the knowledge corpus, not necessarily a whole article.

<sup>&</sup>lt;sup>2</sup>Strictly speaking, MLM is not a standard language model, since it does not define a distribution over the entire sequence of tokens. In the paper we sometimes abuse the term "language model" slightly to make the phrase shorter.
...
$$\begin{split} p(z \mid x) &= \frac{\exp f(x, z)}{\sum_{z'} \exp f(x, z')}, \\ f(x, z) &= \texttt{Embed}_{\texttt{input}}(x)^{\top} \texttt{Embed}_{\texttt{doc}}(z), \end{split}$$

where  ${\tt Embed_{input}}$  and  ${\tt Embed_{doc}}$  are embedding functions that map x and z respectively to d-dimensional vectors. The  ${\tt relevance\ score\ } f(x,z)$  between x and z is defined as the inner product of the vector embeddings. The retrieval distribution is the softmax over all relevance scores.
...
$$\begin{aligned} & \mathtt{join_{BERT}}(x) = \texttt{[CLS]}x\texttt{[SEP]} \\ & \mathtt{join_{BERT}}(x_1, x_2) = \texttt{[CLS]}x_1\texttt{[SEP]}x_2\texttt{[SEP]} \end{aligned}$$

As in Devlin et al. (2018), we pass this into a Transformer, which produces one vector for each token, including the vector corresponding to [CLS] which is used as a "pooled" representation of the sequence (denoted BERT<sub>CLS</sub>). Finally, we perform a linear projection to reduce the dimensionality of the vector, denoted as a projection matrix **W**:

$$\begin{split} &\texttt{Embed}_{\texttt{input}}(x) = \mathbf{W}_{\texttt{input}} \texttt{BERT}_{\texttt{CLS}}(\texttt{join}_{\texttt{BERT}}(x)) \\ &\texttt{Embed}_{\texttt{doc}}(z) = \mathbf{W}_{\texttt{doc}} \texttt{BERT}_{\texttt{CLS}}(\texttt{join}_{\texttt{BERT}}(z_{\texttt{title}}, z_{\texttt{body}})) \end{split}$$
...
$$\begin{split} p(y \mid z, x) &= \prod_{j=1}^{J_x} p(y_j \mid z, x) \\ p(y_j \mid z, x) &\propto \exp\left(w_j^{\top} \text{BERT}_{\text{MASK}(j)}(\text{join}_{\text{BFRT}}(x, z_{\text{body}}))\right) \end{split}$$

where  $\mathrm{BERT}_{\mathtt{MASK}(j)}$  denotes the Transformer output vector corresponding to the  $j^{th}$  masked token,  $J_x$  is the total number of [MASK] tokens in x, and  $w_j$  is a learned word embedding for token  $y_j$ .
...
$$\begin{split} p(y \,|\, z, x) &\propto \sum_{s \in S(z, y)} \exp\left(\text{MLP}\left(\left[h_{\texttt{START}(\textbf{s})}; h_{\texttt{END}(\textbf{s})}\right]\right)\right) \\ h_{\texttt{START}(\textbf{s})} &= \texttt{BERT}_{\texttt{START}(\textbf{s})}(\texttt{join}_{\texttt{BERT}}(x, z_{\texttt{body}})), \\ h_{\texttt{END}(\textbf{s})} &= \texttt{BERT}_{\texttt{END}(\textbf{s})}(\texttt{join}_{\texttt{BERT}}(x, z_{\texttt{body}})), \end{split}$$

where  $\mathrm{BERT}_{\mathtt{START}(\mathbf{s})}$  and  $\mathrm{BERT}_{\mathtt{END}(\mathbf{s})}$  denote the Transformer output vectors corresponding to the start and end tokens of span s, respectively, while MLP denotes a feed-forward neural network. We will let  $\phi$  denote all parameters associated with the knowledge-augmented encoder.

#### 3.3. Training

For both pre-training and fine-tuning, we train by maximizing the log-likelihood  $\log p(y \mid x)$  of the correct output y. Since both the knowledge retriever and knowledge-augmented encoder are differentiable neural networks, we can compute the gradient of  $\log p(y \mid x)$  (defined in Equation 1) with respect to the model parameters  $\theta$  and  $\phi$ , and optimize using stochastic gradient descent.
...
Even with this approximation, we still need an efficient way to find the top k documents. Note that the ordering of documents under  $p(z \mid x)$  is the same as under the relevance score  $f(x,z) = \text{Embed}_{\text{input}}(x)^{\top} \text{Embed}_{\text{doc}}(z)$ , which is an inner product. Thus, we can employ Maximum Inner Product Search (MIPS) algorithms to find the approximate top k documents, using running time and storage space that scale sub-linearly with the number of documents (Ram & Gray, 2012; Shrivastava & Li, 2014; Shen et al., 2015).

To employ MIPS, we must pre-compute  $\mathrm{Embed_{doc}}(z)$  for every  $z \in \mathcal{Z}$  and construct an efficient search index over these embeddings. However, this data structure will no longer be consistent with  $p(z\,|\,x)$  if the parameters  $\theta$  of  $\mathrm{Embed_{doc}}$  are later updated. Hence, the search index goes "stale" after every gradient update on  $\theta$ .
...
below, the trainer sends the index builder a snapshot of its parameters,  $\theta'$ . The trainer then continues to train while the index builder uses  $\theta'$  to construct a new index in the background. As soon as the index builder is done, it sends the new index back to the trainer, and the process repeats.

While asynchronous refreshes can be used for both pretraining and fine-tuning, in our experiments we only use it for pre-training. For fine-tuning, we just build the MIPS index once (using the pre-trained  $\theta$ ) for simplicity and do not update Embed<sub>doc</sub>.<sup>3</sup> Note that we still fine-tune Embed<sub>input</sub>, so the retrieval function is still updated from the query side.
...
For each document z, the gradient encourages the retriever to change the score f(x,z) by r(z) — increasing if r(z) is positive, and decreasing if negative. The multiplier r(z) is positive if and only if  $p(y \mid z, x) > p(y \mid x)$ . The term  $p(y \mid z, x)$  is the probability of predicting the correct output y when using document z. The term  $p(y \mid x)$  is the expected value of  $p(y \mid x, z)$  when randomly sampling a document from  $p(z \mid x)$ . Hence, document z receives a positive update whenever it performs better than expected.
...
<sup>&</sup>lt;sup>4</sup>We initially conducted our own T5 experiments using the code from https://tinyurl.com/t5-openqa-colab (Raffel et al., 2019). We now report results from the concurrent work of Roberts et al. (2020), which has an improved fine-tuning procedure.
...
| Name                                                                                                                                                         | Architectures                                                                                                                            | Pre-training                                       | <b>NQ</b> (79k/4k)           | <b>WQ</b> (3k/2k)         | <b>CT</b> (1k /1k)          | # params                            |
|--------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------|------------------------------|---------------------------|-----------------------------|-------------------------------------|
| BERT-Baseline (Lee et al., 2019)                                                                                                                             | Sparse Retr.+Transformer                                                                                                                 | BERT                                               | 26.5                         | 17.7                      | 21.3                        | 110m                                |
| T5 (base) (Roberts et al., 2020)<br>T5 (large) (Roberts et al., 2020)<br>T5 (11b) (Roberts et al., 2020)                                                     | Transformer Seq2Seq<br>Transformer Seq2Seq<br>Transformer Seq2Seq                                                                        | T5 (Multitask)<br>T5 (Multitask)<br>T5 (Multitask) | 27.0<br>29.8<br>34.5         | 29.1<br>32.2<br>37.4      | -<br>-<br>-                 | 223m<br>738m<br>11318m              |
| DrQA (Chen et al., 2017)<br>HardEM (Min et al., 2019a)<br>GraphRetriever (Min et al., 2019b)<br>PathRetriever (Asai et al., 2019)<br>ORQA (Lee et al., 2019) | Sparse Retr.+DocReader<br>Sparse Retr.+Transformer<br>GraphRetriever+Transformer<br>PathRetriever+Transformer<br>Dense Retr.+Transformer | N/A<br>BERT<br>BERT<br>MLM<br>ICT+BERT             | 28.1<br>31.8<br>32.6<br>33.3 | 20.7<br>31.6<br>-<br>36.4 | 25.7<br>-<br>-<br>-<br>30.1 | 34m<br>110m<br>110m<br>110m<br>330m |
| Ours ( $\mathcal{X}$ = Wikipedia, $\mathcal{Z}$ = Wikipedia)<br>Ours ( $\mathcal{X}$ = CC-News, $\mathcal{Z}$ = Wikipedia)                                   | Dense Retr.+Transformer<br>Dense Retr.+Transformer                                                                                       | REALM<br>REALM                                     | 39.2<br><b>40.4</b>          | 40.2<br><b>40.7</b>       | <b>46.8</b> 42.9            | 330m<br>330m                        |
...
|-----------------------------------------------------------------------------------------|----------------------|------------------------------------|
| REALM                                                                                   | 38.2                 | 38.5                               |
| REALM retriever+Baseline encoder<br>Baseline retriever+REALM encoder<br>Baseline (ORQA) | 37.4<br>35.3<br>31.3 | 38.5<br>13.9<br>13.9               |
| REALM with random uniform masks<br>REALM with random span masks                         | 32.3<br>35.3         | 24.2<br>26.1                       |
...
| x:        | x: An equilateral triangle is easily constructed using a straightedge and compass, because 3 is a prime. |                                                                                                                                                    |  |
|-----------|----------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------|--|
| (a) BERT  | $p(y=\text{``Fermat''} x)  = \ 1.1\times 10^{-14}$                                                       | (No retrieval.)                                                                                                                                    |  |
| (b) REALM | p(y = ``Fermat''    x, z)  =  1.0                                                                        | (Conditional probability with document $z = \text{``257}$ is a Fermat prime. Thus a regular polygon with 257 sides is constructible with compass") |  |
...
- Lee, K., Chang, M.-W., and Toutanova, K. Latent retrieval for weakly supervised open domain question answering. In *Proceedings of the Conference of Associa*tion for Computational Linguistics, 2019.
...
- Seo, M., Kembhavi, A., Farhadi, A., and Hajishirzi, H. Bidirectional attention flow for machine comprehension. In *International Conference on Learning Representa*tions, 2016.
...
$$\begin{split} \nabla \log p(z \mid x) &= \nabla \log \frac{\exp f(x, z)}{\sum_{z'} \exp f(x, z')} \\ &= \nabla \left[ f(x, z) - \log \sum_{z'} \exp f(x, z') \right] \\ &= \nabla f(x, z) - \sum_{z'} p(z' \mid x) \nabla f(x, z') \end{split}$$

Plugging this back into the first set of equations yields:

$$\begin{split} \nabla \log p\left(y\,|\,x\right) &= \sum_{z} p\left(z\,|\,y,x\right) \left[\nabla f(x,z) - \sum_{z'} p\left(z'\,|\,x\right) \nabla f(x,z')\right] \\ &= \sum_{z} p\left(z\,|\,y,x\right) \nabla f(x,z) - \sum_{z'} p\left(z'\,|\,x\right) \nabla f(x,z') \\ &= \sum_{z} \left[p\left(z\,|\,y,x\right) - p\left(z\,|\,x\right)\right] \nabla f(x,z) \\ &= \sum_{z} \left[\frac{p\left(y\,|\,z,x\right) p\left(z\,|\,x\right)}{p\left(y\,|\,x\right)} - p\left(z\,|\,x\right)\right] \nabla f(x,z) \\ &= \sum_{z} \left[\frac{p\left(y\,|\,z,x\right) p\left(z\,|\,x\right)}{p\left(y\,|\,x\right)} - 1\right] p\left(z\,|\,x\right) \nabla f(x,z). \end{split}$$

In the second line, we used the fact that the overall expression is an expectation with respect to  $p(z \mid y, x)$ , and the terms which depend on z' but not z can be moved out of that expectation.

# **B.** Connection between REALM and supervised learning

From the equations in Appendix A, we saw that

$$\nabla \log p(y \mid x) = \sum_{z} \left[ p(z \mid y, x) - p(z \mid x) \right] \nabla f(x, z).$$
...
zero accuracy (i.e.,  $p\left(y\,|\,z',x\right)=0$ ). Under this setting,  $p\left(z^*\,|\,y,x\right)=1$  (provided that  $p\left(z^*\,|\,x\right)$  is non-zero), which causes the gradient to become

$$\begin{split} \nabla \log p\left(y\,|\,x\right) &= \nabla f\left(x,z^*\right) - \sum_{z} p\left(z\,|\,x\right) \nabla f(x,z) \\ &= \nabla \log p\left(z^*\,|\,x\right). \end{split}$$
...
$$RU(z \mid x) = \log p(y \mid z, x) - \log p(y \mid \varnothing, x). \tag{2}$$
...
| x:    | "Jennifer forme                                       | d the production company Excellent Cadaver."                 |
|-------|-------------------------------------------------------|--------------------------------------------------------------|
| BERT  |                                                       | also $(0.13)$ , then $(0.08)$ , later $(0.05)$ ,             |
| REALM | $(\mathcal{Z} = 20 \text{ Dec } 2018 \text{ corpus})$ | smith (0.01), brown (0.01), jones (0.01)                     |
| REALM | $(\mathcal{Z} = 20 \text{ Jan } 2020 \text{ corpus})$ | <b>lawrence</b> $(0.13)$ , brown $(0.01)$ , smith $(0.01)$ , |
