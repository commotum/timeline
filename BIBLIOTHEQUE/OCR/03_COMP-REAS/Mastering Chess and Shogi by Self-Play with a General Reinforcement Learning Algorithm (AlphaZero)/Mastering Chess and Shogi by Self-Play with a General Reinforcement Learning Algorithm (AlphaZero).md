...
Instead of a handcrafted evaluation function and move ordering heuristics, AlphaZero utilises a deep neural network  $(\mathbf{p}, v) = f_{\theta}(s)$  with parameters  $\theta$ . This neural network takes the board position s as an input and outputs a vector of move probabilities  $\mathbf{p}$  with components  $p_a = Pr(a|s)$ 

for each action a, and a scalar value v estimating the expected outcome z from position s,  $v \approx \mathbb{E}[z|s]$ . AlphaZero learns these move probabilities and value estimates entirely from selfplay; these are then used to guide its search.
...
<sup>&</sup>lt;sup>1</sup>The original *AlphaGo Zero* paper used GPUs to train the neural networks.

<sup>&</sup>lt;sup>2</sup>AlphaGo Master and AlphaGo Zero were ultimately trained for 100 times this length of time; we do not reproduce that effort here.

| Game  | White                  | Black                  | Win   | Draw     | Loss     |
|-------|------------------------|------------------------|-------|----------|----------|
| Chess | AlphaZero<br>Stockfish | Stockfish<br>AlphaZero | 25    | 25<br>47 | 0 0      |
| Shogi | AlphaZero<br>Elmo      | Elmo<br>AlphaZero      | 43 47 | 2 0      | 5<br>3   |
| Go    | AlphaZero<br>AG0 3-day | AG0 3-day<br>AlphaZero | 31 29 | _<br>_   | 19<br>21 |
...
<sup>&</sup>lt;sup>3</sup>The prevalence of draws in high-level chess tends to compress the Elo scale, compared to shogi or Go.
