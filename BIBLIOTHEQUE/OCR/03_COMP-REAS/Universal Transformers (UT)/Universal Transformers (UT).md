...
Mostafa Dehghani\*†Stephan Gouws\*Oriol VinyalsUniversity of AmsterdamDeepMindDeepMind

dehghani@uva.nl sgouws@google.com vinyals@google.com

Jakob UszkoreitŁukasz KaiserGoogle BrainGoogle Brain
...
$$MULTIHEADSELFATTENTION(H^t) = CONCAT(head_1,...,head_k)W^O$$
 (2)

where 
$$\text{head}_{i} = \text{ATTENTION}(H^{t}W_{i}^{Q}, H^{t}W_{i}^{K}, H^{t}W_{i}^{V})$$
 (3)
...
$$p(y_{pos}|y_{[1:pos-1]}, H^T) = \text{SOFTMAX}(OH^T)^1$$
 (8)
...
<sup>&</sup>lt;sup>1</sup>Note that T here denotes time-step T and not the transpose operation.

| Model                              | 10K ex       | amples       | 1K examples  |              |  |  |  |  |
|------------------------------------|--------------|--------------|--------------|--------------|--|--|--|--|
|                                    | train single | train joint  | train single | train joint  |  |  |  |  |
| Previous best results:             |              |              |              |              |  |  |  |  |
| QRNet (Seo et al., 2016)           | 0.3 (0/20)   | -            | -            | -            |  |  |  |  |
| Sparse DNC (Rae et al., 2016)      | -            | 2.9 (1/20)   | _            | -            |  |  |  |  |
...
<sup>&</sup>lt;sup>2</sup>We experimented with different hyper-parameters and different network sizes, but it always overfits.
...
<sup>&</sup>lt;sup>4</sup>Cabinet (singular) is an agreement attractor in this case.

| Model                                          | Number of attractors |        |         |       |       |       |       |  |
|------------------------------------------------|----------------------|--------|---------|-------|-------|-------|-------|--|
| 1,10401                                        | 0                    | 1      | 2       | 3     | 4     | 5     | Total |  |
...
| Neural Cache (Grave et al., 2016)    | 129                        | 139               | -                 | -           | -      | _      |
...
| UT w/ dynamic halting                | 130 (0.32)                 | <b>134</b> (0.22) | <b>142</b> (0.19) | 0.4603      | 0.5831 | 0.5625 |
...
| Model                 | Co       | py Reverse |          | erse    | Addition |         |
|-----------------------|----------|------------|----------|---------|----------|---------|
...
<sup>&</sup>lt;sup>5</sup>Note that in UT both the self-attention and transition weights are tied across layers.

<sup>&</sup>lt;sup>6</sup>Appendix B illustrates how UT is computationally more powerful than the standard Transformer.
...
# Output Probabilities Softmax After T steps Recurrent Layer Normalization Decoder Block Dropout Transition Function Recurrent Layer Normalization Layer Normalization Encoder Block Dropout Dropout After T steps Transition Function Multihead Attention Laver Normalization Layer Normalization For T steps Dropout Dropout Multihead Self-Attention Multihead Self-Attention Timestep embedding Timestep embedding Position embedding Position embedding **Embed Input Symbols Embed Target Symbols** Input Sequence Target Sequence (right-shifted by one)
...
16
      # Add the halting probability for this step to the halting # probabilities for those inputs which haven't halted yet
17
18
...
32
33
      transformed state = transition function(self attention(state))
34
...
8http://clic.cimec.unitn.it/lambada/appendix\_onefile.pdf
...
|----------------------------------------------|--------------|-------------|--------------|-------------|--|--|
| Task id                                      | 10           | K           | 1K           |             |  |  |
| rush ru                                      | train single | train joint | train single | train joint |  |  |
| 1                                            | 0.0          | 0.0         | 0.0          | 0.0         |  |  |
| 2                                            | 0.0          | 0.0         | 0.0          | 0.5         |  |  |
| 3                                            | 0.4          | 1.2         | 3.7          | 5.4         |  |  |
| 4                                            | 0.0          | 0.0         | 0.0          | 0.0         |  |  |
...
|---------------------------------------------|-----------------|-----------------|-----------------|-----------------|--|--|--|
| Task id                                     | 10              | K               | 1K              |                 |  |  |  |
| 1401110                                     | train single    | train joint     | train single    | train joint     |  |  |  |
| 1                                           | $0.0 \pm 0.0$   | $0.0 \pm 0.0$   | $0.2 \pm 0.3$   | 0.1 ±0.2        |  |  |  |
...
| 5                                           | $0.2 \pm 0.3$   | $0.8\pm0.5$     | $1.1\pm1.3$     | $4.3 \pm 5.6$   |  |  |  |
...
| 9                                           | $0.0\pm0.0$     | $0.0\pm0.0$     | $0.1 \pm 0.1$   | $0.3 \pm 0.3$   |  |  |  |
| 10                                          | $0.1 \pm 0.2$   | $0.5 \pm 0.4$   | $0.7 \pm 0.8$   | $1.3 \pm 1.6$   |  |  |  |
| 11                                          | $0.0 \pm 0.0$   | $0.1 \pm 0.1$   | $0.4 \pm 0.8$   | $0.3 \pm 0.9$   |  |  |  |
| 12                                          | $0.2 \pm 0.1$   | $0.4\pm0.4$     | $0.6 \pm 0.9$   | $0.3 \pm 0.4$   |  |  |  |
| 13                                          | $0.2\pm0.5$     | $0.3 \pm 0.4$   | $0.8 \pm 0.9$   | $1.1 \pm 0.9$   |  |  |  |
...
| 17                                          | $1.6 \pm 0.8$   | $1.4 \pm 3.4$   | $43.7 \pm 18.6$ | $51.1 \pm 12.9$ |  |  |  |
| 18                                          | $0.3 \pm 0.4$   | $0.7\pm1.4$     | $2.3 \pm 3.6$   | $12.8 \pm 9.0$  |  |  |  |
| 19                                          | $3.4 \pm 4.0$   | $6.1 \pm 7.3$   | $50.2 \pm 8.4$  | $73.1 \pm 23.9$ |  |  |  |
| 20                                          | $0.0\pm0.0$     | $0.0\pm0.0$     | $3.2 \pm 2.5$   | $2.6\pm\!2.8$   |  |  |  |
| avg                                         | $0.73 \pm 0.89$ | $1.12 \pm 1.62$ | $6.34 \pm 3.32$ | 11.21 ±6.65     |  |  |  |
...
Story: Sandra journeyed to the hallway. Mary went to the bathroom. Mary took the apple there. Mary dropped the apple. Question: Where is the apple? Model's output: bathroom Sandra journeyed to the hallway. Sandra journeyed to the hallway. Mary went to the bathroom. Mary went to the bathroom. Mary took the apple there. Mary took the apple there. Mary dropped the apple. Mary dropped the apple. Where is the apple? Where is the apple? (a) Step 1 Sandra journeyed to the hallway. Sandra journeyed to the hallway. Mary went to the bathroom. Mary went to the bathroom . Mary took the apple there . Mary took the apple there . Mary dropped the apple. Mary dropped the apple. Where is the apple? Where is the apple? (b) Step 2 Sandra journeyed to the hallway. Sandra journeyed to the hallway. Mary went to the bathroom. Mary went to the bathroom . Mary took the apple there . Mary took the apple there. Mary dropped the apple. Mary dropped the apple. Where is the apple? Where is the apple? (c) Step 3
