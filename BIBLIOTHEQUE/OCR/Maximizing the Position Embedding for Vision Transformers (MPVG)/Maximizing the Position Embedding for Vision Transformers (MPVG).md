...
# Wonjun Lee<sup>1,2</sup>, Bumsub Ham<sup>1</sup>, Suhyun Kim<sup>2\*</sup>

<sup>1</sup>Yonsei University, Republic of Korea <sup>2</sup>Korea Institute of Science and Technology, Republic of Korea {velpegor, bumsub.ham}@yonsei.ac.kr, dr.suhyun.kim@gmail.com
...
- We propose a simple yet effective method called MPVG, which maximizes the effect of PE in the GAP method. We show that MPVG leads to better performance in vision transformers.
...
In this section, we first explain the absolute position embedding and then provide a detailed overview of the Layerwise structure (Yu et al. 2023). Next, we introduce PVG, an improved Layer-wise structure, along with MPVG, which

effectively leverages the characteristics of PE in the Layerwise structure.
...
$$x_0 = [x_{\text{cls}}; p^1; p^2; \dots p^N;] + pos,$$
 (1)
...
$$x'_{l} = MSA(LN_{l}(x_{l})) + x_{l} \quad (l = 0 \dots L), \tag{2}$$

$$x_{l+1} = \text{MLP}(LN'_{l}(x'_{l})) + x'_{l} \quad (l = 0 \dots L),$$
 (3)
...
$$x_0 = [x_{\text{cls}}; p^1; p^2; \dots p^N;],$$
 (5)

$$x'_{l} = MSA(LN_{l}(x_{l}) + LN'_{l}(pos_{l})) + x_{l}$$
...
$$\begin{cases}
pos_0 = pos \\
pos_l = LN'_{l-1}(pos_{l-1}) & (l = 1 \dots L)
\end{cases}$$
(7)
...
$$x_0 = [p^1; p^2; \dots p^N;] + pos,$$
 (8)

$$x'_{l} = \begin{cases} MSA(LN_{0}(x_{0})) + x_{0} & \text{if } l = 0\\ MSA(LN_{l}(x_{l}) + LN'_{l}(pos_{l-1})) + x_{l} & \text{if } 1 \leq l \leq L \end{cases}$$
(9)

$$\begin{cases}
pos_0 = pos \\
pos_l = LN'_l(pos_{l-1}) & (l = 1...L)
\end{cases}$$
(10)
...
| Model                 | Method  | #Params (M) | Top-1<br>Acc (%) |
|-----------------------|---------|-------------|------------------|
|                       | Default | 5.717       | 72.14            |
| DeiT-Ti               | LaPE    | 5.721       | 72.94            |
| (Touvron et al. 2021) | PVG     | 5.721       | 73.17            |
|                       | MPVG    | 5.721       | 73.51            |
|                       | Default | 22.050      | 79.81            |
| DeiT-S                | LaPE    | 22.059      | 80.39            |
| (Touvron et al. 2021) | PVG     | 22.058      | 80.38            |
|                       | MPVG    | 22.059      | 80.61            |
|                       | Default | 86.567      | 81.85            |
| DeiT-B                | LaPE    | 86.586      | 82.15            |
| (Touvron et al. 2021) | PVG     | 86.583      | 82.21            |
|                       | MPVG    | 86.584      | 82.42            |
...
|                       | MPVG    | 6.361       | 77.20            |
|                       | Default | 4.310       | 71.76            |
| T2T-ViT-7             | LaPE    | 4.313       | 72.01            |
| (Yuan et al. 2021b)   | PVG     | 4.312       | 71.91            |
|                       | MPVG    | 4.313       | 72.28            |
...
| Model                 | Method  | #Param (M) | Top-1<br>Acc (%) |
|-----------------------|---------|------------|------------------|
|                       | Default | 3.740      | 74.90            |
| ViT-Lite              | LaPE    | 3.744      | 75.52            |
| (Hassani et al. 2022) | PVG     | 3.742      | 76.67            |
|                       | MPVG    | 3.743      | <b>76.87</b>     |
|                       | Default | 4.078      | 83.22            |
| T2T-ViT-7             | LaPE    | 4.082      | 83.41            |
| (Yuan et al. 2021b)   | PVG     | 4.081      | 83.39            |
|                       | MPVG    | 4.081      | 83.51            |
...
| Model          | Pre-trained | Method                         | AP <sup>box</sup> / AP <sup>mask</sup>                                 |
|----------------|-------------|--------------------------------|------------------------------------------------------------------------|
| ViT-Adapter-Ti | DeiT-Ti     | Default<br>LaPE<br>PVG<br>MPVG | 45.9 / 41.0<br>46.2 / 41.2<br>46.1 / 41.2<br><b>46.5</b> / <b>41.4</b> |
...
|----------------|-------------|--------------------------------|-----------------------------------------|
| ViT-Adapter-Ti | DeiT-Ti     | Default<br>LaPE<br>PVG<br>MPVG | 40.55<br>41.42<br>41.07<br><b>41.69</b> |
...
|---------|-----------|-----------------------------------------------------------------|-------------------------|
| DeiT-Ti | MPVG      | $\begin{array}{c c} pos_{11} \\ pos_{8} \\ pos_{5} \end{array}$ | 73.30<br>73.38<br>73.39 |
|         |           | $pos_0$                                                         | 73.51                   |
...
| Model | Layer 0  | Structure<br>Hierarchical | (x+PE)     | Top-1<br>Acc (%)      |
|-------|----------|---------------------------|------------|-----------------------|
| MDVC  | <b>X</b> | <b>√ X</b>                | <b>√</b> ✓ | 73.31<br>73.48        |
| MPVG  | ✓<br>✓   | <b>√</b><br><b>√</b>      | <b>X</b>   | 73.28<br><b>73.51</b> |

Table 6: Structural Differences in MPVG. "Layer 0" denotes whether layer 0 is included when delivering PE to layers. "Hierarchical" denotes whether  $pos_l$  is  $\operatorname{LN}_l'(pos_{l-1})$  or  $\operatorname{LN}_l'(pos_0)$ . " $(x+\operatorname{PE})$ " denotes whether PE is added to the token embedding(x) before entering layer 0 or not.
...
| (Deng et al. 2009)               | CeiT     | 5e-4          | cosine    | 0.05         | 1024       | 300    | 5              |
| (Delig et al. 2009)              | T2T-ViT  | 1e-3          | cosine    | 0.03         | 512        | 300+10 | 10             |
...
| Dataset   | Model          | Framework  | Backbone<br>Prei-train | Crop<br>Size | Optimizer | LR   | Scheduler | Weight Decay | Batch Size |
|-----------|----------------|------------|------------------------|--------------|-----------|------|-----------|--------------|------------|
| COCO 2017 | ViT-Adapter-Ti | Mask R-CNN | DeiT-Ti                | 1024         | AdamW     | 1e-4 | 3x+MS     | 0.05         | 8          |

Table 8: Hyperparameter settings for object detection on COCO 2017.

| Dataset                      | Model          | Framework | Backbone<br>Prei-train | Crop<br>Size | Optimizer | LR    | Scheduler | Weight Decay | Batch Size |
|------------------------------|----------------|-----------|------------------------|--------------|-----------|-------|-----------|--------------|------------|
| ADE20K<br>(Zhou et al. 2019) | ViT-Adapter-Ti | UperNet   | DeiT-Ti                | 512          | AdamW     | 12e-5 | 160K      | 0.01         | 8          |
