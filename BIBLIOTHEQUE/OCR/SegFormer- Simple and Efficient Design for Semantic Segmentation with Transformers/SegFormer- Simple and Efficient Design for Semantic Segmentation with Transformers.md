...
number of categories. In the rest of this section, we detail the proposed encoder and decoder designs and summarize the main differences between our approach and SETR.
...
$$Attention(Q, K, V) = Softmax(\frac{QK^{\mathsf{T}}}{\sqrt{d_{head}}})V.$$
 (1)
...
$$\mathbf{x}_{out} = \text{MLP}(\text{GELU}(\text{Conv}_{3\times3}(\text{MLP}(\mathbf{x}_{in})))) + \mathbf{x}_{in}, \tag{3}$$

where  $\mathbf{x}_{in}$  is the feature from the self-attention module. Mix-FFN mixes a 3 × 3 convolution and an MLP into each FFN. In our experiments, we will show that a 3 × 3 convolution is sufficient to provide positional information for Transformers. In particular, we use depth-wise convolutions for reducing the number of parameters and improving efficiency.
...
$$\begin{split} \hat{F}_{i} &= \operatorname{Linear}(C_{i}, C)(F_{i}), \forall i \\ \hat{F}_{i} &= \operatorname{Upsample}(\frac{W}{4} \times \frac{W}{4})(\hat{F}_{i}), \forall i \\ F &= \operatorname{Linear}(4C, C)(\operatorname{Concat}(\hat{F}_{i})), \forall i \\ M &= \operatorname{Linear}(C, N_{cls})(F), \end{split} \tag{4}$$
...
|----------------|---------|----------------------------------|--------|-------------|---------------|-------------|------------|------|
| Model Size     | Encoder | oder Decoder Flops↓ mIoU(SS/MS)↑ |        | Flops ↓     | mIoU(SS/MS) ↑ | Flops ↓     | mIoU(SS) ↑ |      |
...
(b) Accuracy as a function of the MLP (c) Mix-FFN vs. positional encoding (PE) for (d) Accuracy on ADE20K of CNN and dimension  $\tilde{C}$  in the decoder on ADE20K. different test resolution on Cityscapes.

Transformer encoder with MLP decoder. "S4" means stage-4 feature.

| C    | Flops ↓ | Params $\downarrow$ | mIoU ↑ |
|------|---------|---------------------|--------|
| 256  | 25.7    | 24.7                | 44.9   |
| 512  | 39.8    | 25.8                | 45.0   |
| 768  | 62.4    | 27.5                | 45.4   |
...
| Inf Res            | Enc Type | $m Io U \uparrow$ |
|--------------------|----------|-------------------|
| 768×768            | PE       | 77.3              |
...
|               | ICNet [11]           | -               | -        | -       | -      | -      | -          | 30.3  | 67.7   |  |
| ne            | PSPNet [17]          | MobileNetV2     | 13.7     | 52.9    | 57.7   | 29.6   | 423.4      | 11.2  | 70.2   |  |
| Ξ             | DeepLabV3+ [20]      | MobileNetV2     | 15.4     | 69.4    | 43.1   | 34.0   | 555.4      | 8.4   | 75.2   |  |
| Real-Time     |                      |                 |          | 8.4     | 50.5   | 37.4   | 125.5      | 15.2  | 76.2   |  |
| 2             | CasEsaman (O         | M:T DO          | 3.8      | -       | -      | -      | 51.7       | 26.3  | 75.3   |  |
|               | SegFormer (Ours)     | MiT-B0          | 3.8      | -       | -      | -      | 31.5       | 37.1  | 73.7   |  |
|               |                      |                 |          | -       | -      | -      | 17.7       | 47.6  | 71.9   |  |
...
| <u>e</u>      | CCNet [41]           | ResNet-101      | 68.9     | 278.4   | 14.1   | 45.2   | 2224.8     | 1.0   | 80.2   |  |
| E             | DeeplabV3+ [20]      | ResNet-101      | 62.7     | 255.1   | 14.1   | 44.1   | 2032.3     | 1.2   | 80.9   |  |
| '=            | OCRNet [23]          | HRNet-W48       | 70.5     | 164.8   | 17.0   | 45.6   | 1296.8     | 4.2   | 81.1   |  |
| Re            | GSCNN [35]           | WideResNet38    | -        | -       | -      | -      | -          | -     | 80.8   |  |
| Non Real-Time | Axial-DeepLab [74]   | AxialResNet-XL  | -        | -       | -      | -      | 2446.8     | -     | 81.1   |  |
| ž             | Dynamic Routing [75] | Dynamic-L33-PSP | -        | -       | -      | -      | 270.0      | -     | 80.7   |  |
...
COCO-Stuff. Finally, we evaluate SegFormer on the Table 4: Results on COCO-Stuff full dataset containing full COCO-Stuff dataset. For comparison, as existing methods do not provide results on this dataset, we reproduce the most representative methods such as DeeplabV3+, OCRNet, and SETR. In this case, the flops on this dataset are the same as those reported for ADE20K. As shown in Table 4, SegFormer-B5 reaches 46.7% mIoU with only 84.7M parameters, which is 0.9%
...
better and 4× smaller than SETR. In summary, these results demonstrate the superiority of SegFormer in semantic segmentation in terms of accuracy, computation cost, and model size.

## 4.4 Robustness to natural corruptions

Model robustness is important for many safety-critical tasks such as autonomous driving [77]. In this experiment, we evaluate the robustness of SegFormer to common corruptions and perturbations. To
...
| Method       | Clean |        | Blı   | ır    |       |       | Noi   | ise  |       |        | Dig   | ital  |      |      | Weat  | ther |       |
|--------------|-------|--------|-------|-------|-------|-------|-------|------|-------|--------|-------|-------|------|------|-------|------|-------|
| Method       | Clean | Motion | Defoc | Glass | Gauss | Gauss | Impul | Shot | Speck | Bright | Contr | Satur | JPEG | Snow | Spatt | Fog  | Frost |
...
|         | Output Size                          | Layer Name             |                                      |                     | Mix Tra                                                        | nsformer                                |                     |                                                  |  |  |  |
|---------|--------------------------------------|------------------------|--------------------------------------|---------------------|----------------------------------------------------------------|-----------------------------------------|---------------------|--------------------------------------------------|--|--|--|
|         | - · · · · ·                          | .,                     | В0                                   | B1                  | B2                                                             | В3                                      | B4                  | В5                                               |  |  |  |
|         |                                      | Overlapping            | $K_1 = 7; \ S_1 = 4; \ P_1 = 3$      |                     |                                                                |                                         |                     |                                                  |  |  |  |
| Stage 1 | $\frac{H}{4} \times \frac{W}{4}$     | Patch Embedding        | $C_1 = 32$                           |                     |                                                                | $C_1 = 64$                              |                     |                                                  |  |  |  |
| Stage 1 | $\frac{1}{4}$ $\wedge$ $\frac{1}{4}$ | Transformer<br>Encoder | $R_1 = 8  N_1 = 1  E_1 = 8  L_1 = 2$ | $N_1 = 1$ $E_1 = 8$ | $   \begin{aligned}   N_1 &= 1 \\   E_1 &= 8   \end{aligned} $ |                                         | $E_1 = 8$           | $R_1 = 8$ $N_1 = 1$ $E_1 = 4$ $L_1 = 3$          |  |  |  |
| 1       |                                      | Overlapping            |                                      | $K_{2}$             | $S_2 = 3; S_2$                                                 | $= 2; P_2 =$                            | : 1                 |                                                  |  |  |  |
| Stage 2 | $\frac{H}{8} \times \frac{W}{8}$     | Patch Embedding        | $C_2 = 64$                           |                     |                                                                | $C_2 = 128$                             | 3                   |                                                  |  |  |  |
| Stage 2 | 8 ^ 8                                | Transformer<br>Encoder |                                      | $N_2 = 2$ $E_2 = 8$ | $ \begin{aligned} N_2 &= 2 \\ E_2 &= 8 \end{aligned} $         | $R_2 = 4$ $N_2 = 2$ $E_2 = 8$ $L_2 = 3$ | $N_2 = 2$ $E_2 = 8$ | $R_2 = 4$ $N_2 = 2$ $E_2 = 4$ $L_2 = 6$          |  |  |  |
| 1       |                                      | Overlapping            | $K_3 = 3; S_3 = 2; P_3 = 1$          |                     |                                                                |                                         |                     |                                                  |  |  |  |
| Stage 3 | $\frac{H}{16} \times \frac{W}{16}$   | Patch Embedding        | $C_3 = 160$                          |                     |                                                                | $C_3 = 320$                             | )                   |                                                  |  |  |  |
| Stage 3 | 16 ^ 16                              | Transformer<br>Encoder |                                      | $N_3 = 5$ $E_3 = 4$ | $N_3 = 5$ $E_3 = 4$                                            | $N_3 = 5$                               | $E_3 = 4$           | $N_3 = 5$                                        |  |  |  |
|         |                                      | Overlapping            |                                      | $K_{\cdot}$         | $s_4 = 3; S_4$                                                 | $= 2; P_4 =$                            | : 1                 |                                                  |  |  |  |
| Stage 4 | $H \vee W$                           | Patch Embedding        | $C_4 = 256$                          |                     |                                                                | $C_4 = 512$                             | 2                   |                                                  |  |  |  |
|         | 32 ^ 32                              | Transformer<br>Encoder | $R_4 = 1  N_4 = 8  E_4 = 4  L_4 = 2$ |                     | $N_4 = 8$ $E_4 = 4$                                            | $R_4 = 1$ $N_4 = 8$ $E_4 = 4$ $L_4 = 3$ |                     | $R_4 = 1$<br>$N_4 = 8$<br>$E_4 = 4$<br>$L_4 = 3$ |  |  |  |
...
- [3] Hang Zhang, Chongruo Wu, Zhongyue Zhang, Yi Zhu, Haibin Lin, Zhi Zhang, Yue Sun, Tong He, Jonas Mueller, R Manmatha, et al. ResNest: Split-attention networks. *arXiv*, 2020.
...
- [18] Maoke Yang, Kun Yu, Chi Zhang, Zhiwei Li, and Kuiyuan Yang. Denseaspp for semantic segmentation in street scenes. In CVPR, 2018.
...
- [22] Changqian Yu, Jingbo Wang, Changxin Gao, Gang Yu, Chunhua Shen, and Nong Sang. Context prior for scene segmentation. In *CVPR*, 2020.

- [23] Yuhui Yuan, Xilin Chen, and Jingdong Wang. Object-contextual representations for semantic segmentation. arXiv, 2019. 7, 8
...
- [43] Hengshuang Zhao, Yi Zhang, Shu Liu, Jianping Shi, Chen Change Loy, Dahua Lin, and Jiaya Jia. Psanet: Point-wise spatial attention network for scene parsing. In *ECCV*, 2018. 8
- [44] Xia Li, Zhisheng Zhong, Jianlong Wu, Yibo Yang, Zhouchen Lin, and Hong Liu. Expectation-maximization attention networks for semantic segmentation. In *ICCV*, 2019.
...
- [53] Li Yuan, Yunpeng Chen, Tao Wang, Weihao Yu, Yujun Shi, Zihang Jiang, Francis EH Tay, Jiashi Feng, and Shuicheng Yan. Tokens-to-token vit: Training vision transformers from scratch on imagenet. arXiv, 2021.
...
- [57] Yawei Li, Kai Zhang, Jiezhang Cao, Radu Timofte, and Luc Van Gool. Localvit: Bringing locality to vision transformers. *arXiv*, 2021. 3
...
- [62] Tim Meinhardt, Alexander Kirillov, Laura Leal-Taixe, and Christoph Feichtenhofer. Trackformer: Multiobject tracking with transformers. arXiv, 2021. 3
...
- [67] Alec Radford, Jong Wook Kim, Chris Hallacy, Aditya Ramesh, Gabriel Goh, Sandhini Agarwal, Girish Sastry, Amanda Askell, Pamela Mishkin, Jack Clark, et al. Learning transferable visual models from natural language supervision. arXiv, 2021. 3

- [68] Ronghang Hu and Amanpreet Singh. Transformer is all you need: Multimodal multitask learning with a unified transformer. *arXiv*, 2021. 3
