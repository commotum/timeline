...
$$\operatorname{Attention}(Q, K, V) = \operatorname{softmax}\left(\frac{QK^T}{\sqrt{d_k}}\right)V, \tag{2}$$

where  $\frac{QK^T}{\sqrt{d_k}} \in \mathbb{R}^{n \times n}$  represents the scaled dot-product similarity matrix, and softmax $(\cdot)$  ensures the attention weights are no-negative and sum to 1 across each row.
...
$$Y' = g(Y, X, W_{\theta}, \sigma) = Y \odot \sigma(XW_{\theta}), \tag{5}$$
...
|                                                                               |                     | Gating                      | g Granularity Var | iants      |           |       |       |        |  |  |
...
|                                                                               |                     | Head-Specif                 | fic v.s. Head-Sha | red Gating |           |       |       |        |  |  |
| (12) SDPA Head-Shared $G_1$                                                   | sigmoid             | $n \times d_k$              | 201               | 5.801      | 74.34     | 60.06 | 53.15 | 61.01  |  |  |
| $(13)$ v Head-Shared $G_2$                                                    | sigmoid             | $n \times d_k^n$            | 25                | 5.867      | 74.10     | 59.02 | 53.03 | 60.61  |  |  |
...
|-----------------------------------------------------------------|-----------------------------------------------------------------|-------------|-------------------------|-------------|-----------|-----------|--------|-------|--|
| 28 Layer, 1.7B Parameters, <b>400B Tokens</b> , Batch Size=1024 |                                                                 |             |                         |             |           |           |        |       |  |
...
|                                                                 | 28 Layer,                                                       | 1.7B Parame | eters, <b>3.5T Toke</b> | ns, Batch S | Size=2048 |           |        |       |  |
...
| <ul><li>(5) SDPA GroupNorm</li><li>(6) SDPA SiLU</li><li>(7) SDPA Additive Gate</li></ul> | RMSNorm             | 5.847        | 74.10        | 60.15        | 53.75        | 61.14        |
|                                                                                           | SiLU                | 5.975        | 73.34        | 59.55        | 53.19        | 60.90        |
|                                                                                           | Identity            | 5.882        | 74.17        | 59.20        | 52.77        | 59.86        |
...
$$o_i^k = \text{Non-Linearity-Map}\left(\sum\nolimits_{j=0}^{i} S_{ij}^k \cdot X_j W_V^k\right) W_O^k. \tag{8}$$
...
<sup>&</sup>lt;sup>2</sup>Note that concatenating outputs from different heads and then multiplying with  $W_O$  is equivalent to multiplying each head's output with its corresponding  $W_O^k$  before concatenation
