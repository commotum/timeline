...
Bowen Peng<sup>1\*</sup> Jeffrey Quesnelle<sup>1†</sup> Honglu Fan<sup>23</sup> Enrico Shippole<sup>‡</sup>

<sup>1</sup>Nous Research <sup>2</sup>EleutherAI <sup>3</sup>University of Geneva
...
<sup>\*</sup>Reddit: /u/bloc97 GitHub: bloc97

<sup>†</sup>Reddit: /u/emozilla X: @theemozilla GitHub: jquesnelle

 $<sup>^{\</sup>ddagger}\mathrm{X}$ : @EnricoShippole GitHub: conceptofmind
...
$$\operatorname{softmax}(\frac{\mathbf{q}_{m}^{T}\mathbf{k}_{n}}{\sqrt{|D|}}),\tag{2}$$
...
$$((\mathbf{x}_m)_1, \cdots, (\mathbf{x}_m)_{|D|}) \mapsto ((\mathbf{x}_m)_1 + i(\mathbf{x}_m)_2, \cdots, ((\mathbf{x}_m)_{|D|-1} + i(\mathbf{x}_m)_{|D|})), \tag{3}$$

$$((\mathbf{q}_m)_1, \cdots, (\mathbf{q}_m)_{|D|}) \mapsto ((\mathbf{q}_m)_1 + i(\mathbf{q}_m)_2, \cdots, ((\mathbf{q}_m)_{|D|-1} + i(\mathbf{q}_m)_{|D|})). \tag{4}$$

To convert embeddings  $\mathbf{x}_m$ ,  $\mathbf{x}_n$  into query and key vectors, we are first given  $\mathbb{R}$ -linear operators

$$\mathbf{W}_a, \mathbf{W}_k : \mathbb{R}^{|D|} \to \mathbb{R}^{|D|}.$$

In complex coordinates, the functions  $f_q$ ,  $f_k$  are given by

$$f_q(\mathbf{x}_m, m) = e^{im\theta} \mathbf{W}_q \mathbf{x}_m, \ f_k(\mathbf{x}_n, n) = e^{in\theta} \mathbf{W}_k \mathbf{x}_n,$$
 (5)

where  $\theta = \operatorname{diag}(\theta_1, \cdots, \theta_{|D|/2})$  is the diagonal matrix with  $\theta_d = b^{-2d/|D|}$  and b = 10000. This way, RoPE associates each (complex-valued) hidden neuron with a separate frequency  $\theta_d$ . The benefit of doing so is that the dot product between the query vector and the key vector only depends on the relative distance m-n as follows

$$\langle f_q(\mathbf{x}_m, m), f_k(\mathbf{x}_n, n) \rangle_{\mathbb{R}}$$
 (6)

$$= \operatorname{Re}(\langle f_q(\mathbf{x}_m, m), f_k(\mathbf{x}_n, n) \rangle_{\mathbb{C}}) \tag{7}$$

$$=\operatorname{Re}(\mathbf{x}_{m}^{*}\mathbf{W}_{q}^{*}\mathbf{W}_{k}\mathbf{x}_{n}e^{i\theta(m-n)})$$
(8)
...
In this section, we think heavily in terms of the wavelengths  $\lambda_d$  defined in Eq. 13 in the formula of RoPE. For simplicity, we omit the subscript d in  $\lambda_d$  and the reader is encouraged to think about  $\lambda$  as the wavelength of an arbitrary periodic function.
...
**Definition 2** The "NTK-by-parts" interpolation is a modification of RoPE by using Eq. 12 with the following functions<sup>4</sup>.

$$q(m) = m (19)$$

$$h(\theta_d) = \left(1 - \gamma(r(d))\right) \frac{\theta_d}{s} + \gamma(r(d))\theta_d. \tag{20}$$
...
<sup>&</sup>lt;sup>4</sup>The interpolation by linear ramp on h may have alternatives, such as a harmonic mean over  $\theta_d/s$  and  $\theta_d$  converted from a linear interpolation on wavelengths. The choice of h here was for the simplicity of implementation, but both would work.
...
$$\operatorname{softmax}\left(\frac{\mathbf{q}_{m}^{T}\mathbf{k}_{n}}{t\sqrt{|D|}}\right). \tag{21}$$
...
Table 1 shows a side-by-side comparison of Llama-2 model extended from 4096 to 8192 context length via PI (LLongMA-2 7b<sup>5</sup>), "NTK-aware" and YaRN. Note that PI and "NTK-aware" models were trained using the methodology in Chen et al. [9], while YaRN used the same methodology but 2.5x less training steps and data, as described in 4.

<sup>&</sup>lt;sup>5</sup>LLongMA-2 7b [28] is fine-tuned from Llama-2 7b, trained at 8k context length with PI using the RedPajama dataset [12].

| Extension              | Extension Trained Context Evaluation Context Window Size |        |      |      |      |      |       |  |
|------------------------|----------------------------------------------------------|--------|------|------|------|------|-------|--|
...
| Model<br>Size        | Model<br>Name                                                | Context<br>Window          | Extension<br>Method       | 8192                         | valuation<br>32768           | Context<br>65536            | Window 98304                         | Size<br>131072                  |
|----------------------|--------------------------------------------------------------|----------------------------|---------------------------|------------------------------|------------------------------|-----------------------------|--------------------------------------|---------------------------------|
| 7B<br>7B<br>7B<br>7B | Together<br>Code Llama<br>YaRN $(s = 16)$<br>YaRN $(s = 32)$ | 32k<br>100k<br>64k<br>128k | PI<br>NTK<br>YaRN<br>YaRN | 3.50<br>3.71<br>3.51<br>3.56 | 2.64<br>2.74<br>2.65<br>2.70 | $> 10^2$ 2.55 2.42 2.45     | $> 10^3$ $2.54$ $> 10^1$ <b>2.36</b> | $> 10^4$ $2.71$ $> 10^1$ $2.37$ |
| 13B<br>13B<br>13B    | Code Llama<br>YaRN ( $s = 16$ )<br>YaRN ( $s = 32$ )         | 100k<br>64k<br>128k        | NTK<br>YaRN<br>YaRN       | 3.54<br><b>3.25</b><br>3.29  | 2.63<br><b>2.50</b><br>2.53  | 2.41<br><b>2.29</b><br>2.31 | $2.37$ $> 10^{1}$ <b>2.23</b>        | $2.54$ $> 10^{1}$ <b>2.24</b>   |
...
| Model<br>Size        | Model<br>Name                                                | Context<br>Window          | Extension<br>Method       | ARC-c                        | Hellaswag                           | MMLU                         | TruthfulQA                          |
|----------------------|--------------------------------------------------------------|----------------------------|---------------------------|------------------------------|-------------------------------------|------------------------------|-------------------------------------|
| 7B                   | Llama 2                                                      | 4k                         | None                      | 53.1                         | 77.8                                | 43.8                         | 39.0                                |
| 7B<br>7B<br>7B<br>7B | Together<br>Code Llama<br>YaRN $(s = 16)$<br>YaRN $(s = 32)$ | 32k<br>100k<br>64k<br>128k | PI<br>NTK<br>YaRN<br>YaRN | 47.6<br>39.9<br>52.3<br>52.1 | 76.1<br>60.8<br><b>78.8</b><br>78.4 | 43.3<br>31.1<br>42.5<br>41.7 | <b>39.2</b><br>37.8<br>38.2<br>37.3 |
| 13B                  | Llama 2                                                      | 4k                         | None                      | 59.4                         | 82.1                                | 55.8                         | 37.4                                |
| 13B<br>13B<br>13B    | Code Llama<br>YaRN ( $s = 16$ )<br>YaRN ( $s = 32$ )         | 100k<br>64k<br>128k        | NTK<br>YaRN<br>YaRN       | 40.9<br>58.1<br>58.0         | 63.4<br><b>82.3</b><br>82.2         | 32.8<br>52.8<br>51.9         | <b>43.8</b> 37.8 37.3               |
...
- [6] bloc97. NTK-Aware Scaled RoPE allows LLaMA models to have extended (8k+) context size without any fine-tuning and minimal perplexity degradation., 2023. URL https://www.reddit.com/r/LocalLLaMA/comments/141z7j5/ntkaware\_scaled\_rope\_allows\_llama\_models\_to\_have/.
...
$$b' = b \cdot s^{\frac{|D|}{|D|-2}}. (24)$$

# A.2 The impact of pre-softmax scaling of YaRN on perplexity

In Section 3.4, we mention the impact of the factor t inside the softmax computation of attention weights. Here we fix 896 16k-token documents from RedPajama [12]<sup>6</sup>, and calculate their perplexity scores with different scaling  $1/\sqrt{t}$ . The result is in Figure 2. For comparison, recall that our recommended factor in this case (s=8) is given by the following.

$$\sqrt{\frac{1}{t}} = 0.1 \ln(s) + 1 \approx 1.208. \tag{25}$$

<sup>&</sup>lt;sup>6</sup>We choose RedPajama because it is the open-source dataset closest to the training dataset of LLaMA as far as we are aware of.
...
$$\frac{\operatorname{ppl}(t) - \operatorname{ppl}(t=1)}{\operatorname{ppl}(t=1)} \tag{26}$$
...
Figure 2: Fix s=8, compare the LLaMA 7b perplexity on  $896\ 16$ k-token documents over different scaling  $1/\sqrt{t}$ . The shaded area represents 1 standard deviation (68%).
...
| Model<br>Size | Model<br>Name | Context<br>Window | Extension<br>Method | Perplexity |
|---------------|---------------|-------------------|---------------------|------------|
...
# The sample counts with minimal perplexity at a given $1/\sqrt{t}$ (different lines represent different position segments at the 16k-token samples)
...
| Model<br>Size | Model<br>Name | Scaling Factor $(s)$ | Context<br>Window | Training<br>Data Context | Extension<br>Method | Passkey<br>Context | Passkey<br>Accuracy |
|---------------|---------------|----------------------|-------------------|--------------------------|---------------------|--------------------|---------------------|
