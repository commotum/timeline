...
<sup>•</sup> A. Agrawal, J. Lu and S. Antol are with Virginia Tech.

<sup>•</sup> M. Mitchell is with Microsoft Research, Redmond.

C. L. Zitnick is with Facebook AI Research.

<sup>•</sup> D. Batra and D. Parikh are with Georgia Institute of Technology.
...
**Captions.** The MS COCO dataset [32], [7] already contains five single-sentence captions for all images. We also collected five single-captions for all abstract scenes using the same user interface<sup>1</sup> for collection.
...
The same user interface was used for both the real images and abstract scenes. In total, three questions from unique workers were gathered for each image/scene. When writing a question, the subjects were shown the previous questions already asked for that image to increase the question diversity. In total, the dataset contains over  $\sim 0.76 M$  questions.
...
<sup>2.</sup> In order to be consistent with 'human accuracies' reported in Section 4, machine accuracies are averaged over all  $\binom{10}{9}$  sets of human annotators
...
| <b>3-4</b> (15.3%)          | <b>5-8</b> (39.7%)                  | <b>9-12</b> (28.4%)                                   | <b>13-17</b> (11.2%)                                                       | <b>18+</b> (5.5%)                                               |
|-----------------------------|-------------------------------------|-------------------------------------------------------|----------------------------------------------------------------------------|-----------------------------------------------------------------|
| Is that a bird in the sky?  | How many pizzas are shown?          | Where was this picture taken?                         | Is he likely to get mugged if he walked<br>down a dark alleyway like this? | What type of architecture is this?                              |
...
the "per Q-type prior" baseline, for the multiple-choice task, we pick the answer (from the provided choices) that is most similar to the picked answer for the open-ended task using cosine similarity in
...
- 2) **norm I:** These are  $\ell_2$  normalized activations from the last hidden layer of VGGNet [48].
...
- For BoW Q + I method, we simply concatenate the BoW Q and I embeddings.
...
| Caption                | 26.70 | 65.50  | 02.03  |       | 28.29 | 69.79    | 02.06    | 03.82 |
...
|                     |       | O       | pen-End | ed    |       | Human Age  | Commonsense   |
|---------------------|-------|---------|---------|-------|-------|------------|---------------|
| Question            |       | K = 100 | 00      | Hu    | man   | To Be Able | To Be Able    |
| Туре                | Q     | Q + I   | Q + C   | Q     | Q + I | To Answer  | To Answer (%) |
...
Table 4 shows the accuracy of different ablated versions of our best model (deeper LSTM Q + norm I) for both the openended and multiple-choice tasks on the VQA test-dev for real images. The different ablated versions are as follows –

- 1) Without I Norm: In this model, the activations from the last hidden layer of VGGNet [48] are not  $\ell_2$ -normalized. Comparing the accuracies in Table 4 and Table 2, we can see that  $\ell_2$ -normalization of image features boosts the performance by 0.16% for open-ended task and by 0.24% for multiple-choice task.
...
3)  $\vec{K}$  = 500: In this model, we use K = 500 most frequent answers as possible outputs. Comparing the accuracies in Table 4 and Table 2, we can see that K = 1000 performs better than K = 500 by 0.82% for open-ended task and by 1.92% for multiple-choice task.
...
We have set up an evaluation server<sup>3</sup> where results may be uploaded for the test set and it returns an accuracy breakdown. We are organizing an annual challenge and workshop to facilitate systematic progress in this area; the first instance of the workshop will be held at CVPR 2016<sup>4</sup>. We suggest that papers reporting results on the VQA dataset –
...
| MM_PaloAlto            | 60.36 | 80.43  | 36.82  | 48.33 | _     | _        | _        | _     |
| LV-NUS                 | 59.54 | 81.34  | 35.67  | 46.10 | 64.18 | 81.25    | 38.30    | 55.20 |
| ACVT_Adelaide          | 59.44 | 81.07  | 37.12  | 45.83 | _     | _        | _        | _     |
| global vision          | 58.43 | 78.24  | 36.27  | 46.32 | _     | _        | _        | _     |
| deeper LSTM Q + norm I | 58.16 | 80.56  | 36.53  | 43.73 | 63.09 | 80.59    | 37.70    | 53.64 |
| iBOWIMG                | _     | -      | -      | -     | 61.97 | 76.86    | 37.30    | 54.60 |
...
- "It is a kitchen." -> "kitchen For yes/no questions, please just say yes/no.

  • "You bet it is!" -> "yes"
- For numerical answers, please use digits

  Ten." -> "10"
- If you need to speculate (e.g., "What just happened?"), provide an answer that most people would agree on. If you don't know the answer (e.g., specific dog breed),
- provide your best guess. Respond matter-of-factly and avoid using
- conversational language or inserting your opinion.
...
| Do you think you were able to answer the question correctly?  (Clicking an option will take you to the next question.) | >        |
...
(f) 4 (j) green (n) so people don't get wer (r) ketchup pickle relish mu (e) 3 (i) blue (m) floor mat (q) pharos

(a) yes (c) 1 (g) white (k) 108 mph (o) fruit salad (e) 3 (i) blue (m) 7 (q) 5 (f) 4 (j) green (n) 10 many (r) vattenfal

![](_page_23_Picture_6.jpeg)

#### Q: What sport are they playing?

(a) yes (c) 1 **(b)** no **(d)** 2 **(e)** 3 (f) 4 (i) green
(n) mississippi and meade
(r) its advertising object (i) blue (m) scissors (q) soccer

#### Q: What an in gray pant's job?

**(b)** no

(d) 2 (h) red (l) umpire (e) 3 (i) blue (m) snowflake (q) 10: 32 (f) 4 (j) green (n) banker (c) 1 (g) white (k) cop (o) chef (p) speedboar

![](_page_23_Picture_12.jpeg)

#### O: What is the color of freebee?

(b) no (d) 2 (h) red (l) peach (p) christ (f) 4 (j) green (n) vitamin o (r) black

#### Q: How old is the child?

(b) no (d) 2 (h) red (l) 12 (e) 3 (i) blue (m) 10 (q) mad (g) white (k) 6 (o) 5 (j) green (n) mechanics (r) recording stud

![](_page_23_Picture_17.jpeg)

#### O: Is this person's face painted?

(b) no (d) 2 (h) red (l) not (p) n200 (e) 3 (i) blue (m) camera film (q) public storage (f) 4 (j) green (n) keyboard, mouse (r) pasta, sauce, mea

![](_page_23_Picture_20.jpeg)

(b) no (d) 2 (h) red (l) 54 (a) yes (c) 1 (g) white (k) 20 (o) 12 (e) 3 (i) blue (m) max payne (q) 3 to 5 (j) green (n) 62 (r) two v

![](_page_23_Picture_23.jpeg)

#### Q: How many of the deer are sleeping?

(b) no (d) 2 (h) red (l) left of pond (p) cement (a) yes (c) 1 (g) white (k) 5 (o) tree ba (e) 3 (i) blue (m) 13 (q) 0 (f) 4 (j) yellow (n) plants and cat (r) green, blue and yellow

![](_page_23_Picture_26.jpeg)

#### Q: Where is the blanket?

(a) yes (c) 1 (g) white (k) fat (o) on bed (b) no (d) 2 (h) red (l) lying (p) grass (e) 3 (i) blue (m) bed (q) grou (f) 4 (j) yellow (n) utensils (r) watching child

![](_page_23_Picture_29.jpeg)

#### Q: What type of wildlife is thi

(a) yes (c) 1 (g) white (k) eating (o) birds (b) no (d) 2 (h) red (l) deer (p) ants (f) 4 (j) yellow (n) soup (r) woma (e) 3 (i) blue (m) mos (q) girl's

![](_page_23_Picture_32.jpeg)

#### O: What is for dessert?

(a) yes (c) 1 (g) white (k) cake (o) ice crea (b) no (d) 2 (h) red (l) pie (p) yell (e) 3 (i) blue (m) a (q) che (f) 4 (j) yellow (n) doll and dollhou (r) there are no fish

![](_page_23_Picture_35.jpeg)

(b) no (d) 2 (h) red (a) yes (c) 1 (g) white (f) 4 (j) yellow (n) sitting (r) park ran (k) yes! (o) to sle (I) standi (m) hiding (q) slide

![](_page_23_Picture_38.jpeg)

# Q: Why does the little girl not look happy?

(a) yes (c) 1 (g) white (k) indian (o) corner (b) no (d) 2 (h) red (m) dog left (q) she fell (I) upset (p) to be pe (n) smiling at it (r) boy is playing with her toy

**(e)** 3

(i) blue (m) dogs (q) bored

(f) 4

(j) yellow (n) shelf (r) likes th

![](_page_23_Picture_41.jpeg)

(**b)** no (**d)** 2 (a) yes (c) 1 **(e)** 3 (f) 4 (h) red (l) deer (p) yes 3 of th (g) white (k) fork (o) slide (i) blue (m) rock (q) no im (j) yellow
(n) y
(r) children and toys

![](_page_23_Picture_44.jpeg)

**(b)** no (d) 2 (h) red (l) shee (e) 3 (i) blue (m) waiti (q) forks (f) 4 (j) yellow (n) no where to sit (p) rugby (r) waiting for train

![](_page_23_Picture_46.jpeg)

# Q: How many legs does the dog have?

(a) yes (c) 1

(g) white (k) he likes them (o) he feeds them

(a) yes (c) 1 (g) white (k) outdo (e) 3 (i) blue (m) 45 (q) 12 (f) 4 (j) yellow (n) sitting in grass (o) owls (p) 8 (r) arm of sof

(b) no (d) 2 (h) red (l) parking it (p) lonely

![](_page_23_Picture_49.jpeg)

#### Q: Is the TV on?

(a) yes (c) 1 (g) white (k) shag (o) edinb (**b**) no (**d**) 2 (**h**) red (e) 3 (i) blue (f) 4 (j) yellow (n) between big elephant (r) white streak on face (I) jeopardy (p) strawbe (m) sports (q) tv show

# Q: Is the boy at the top of the ladder?

(a) yes (c) 1 (g) white (k) not sure (o) a (b) no (d) 2 (h) red (l) yellow dog (f) 4 (j) yellow (n) behind trees (r) she's in middle (e) 3 (i) blue (m) botto (q) top

# REFERENCES

- H. Agrawal, C. S. Mathialagan, Y. Goyal, N. Chavali, P. Banik, A. Mohapatra, A. Osman, and D. Batra. Cloudcv: Large-scale distributed computer vision as a cloud service. In *Mobile Cloud Visual Media Computing*, pages 265–290. Springer International Publishing, 2015. 10
- Computing, pages 265–290. Springer International Publishing, 2015. 10
   S. Antol, C. L. Zitnick, and D. Parikh. Zero-Shot Learning via Visual Abstraction. In ECCV, 2014. 2, 3, 17
- [3] J. P. Bigham, C. Jayanf, H. Ji, G. Little, A. Miller, R. C. Miller, R. Miller, A. Tatarowicz, B. White, S. White, and T. Yeh. VizWiz: Nearly Real-time Answers to Visual Questions. In *User Interface Software and Technology*, 2010. 1, 2, 12
- [4] K. Bollacker, C. Evans, P. Paritosh, T. Sturge, and J. Taylor. Freebase: A Collaboratively Created Graph Database for Structuring Human Knowledge. In *International Conference on Management of Data*, 2008.
...
- [22] M. Hodosh, P. Young, and J. Hockenmaier. Framing Image Description as a Ranking Task: Data, Models and Evaluation Metrics. *JAIR*, 2013.
...
- fast feature embedding. *arXiv preprint arXiv:1408.5093*, 2014. 17 [24] A. Karpathy and L. Fei-Fei. Deep Visual-Semantic Alignments for Generating Image Descriptions. In *CVPR*, 2015. 1, 2
- Generating Image Descriptions. In CVPR, 2015. 1, 2
  [25] S. Kazemzadeh, V. Ordonez, M. Matten, and T. L. Berg. ReferItGame:
  Referring to Objects in Photographs of Natural Scenes. In EMNLP,
  2014. 2
- [26] R. Kiros, R. Salakhutdinov, and R. S. Zemel. Unifying Visual-Semantic Embeddings with Multimodal Neural Language Models. *TACL*, 2015.
...
- [31] D. B. Lenat and R. V. Guha. Building Large Knowledge-Based Systems; Representation and Inference in the Cyc Project. Addison-Wesley Longman Publishing Co., Inc., 1989.
...
- [33] X. Lin and D. Parikh. Don't Just Listen, Use Your Imagination: Leveraging Visual Common Sense for Non-Visual Tasks. In CVPR, 2015.
...
- [37] M. Malinowski, M. Rohrbach, and M. Fritz. Ask your neurons: A neural-based approach to answering questions about images. In *ICCV*, 2015.
- [38] J. Mao, W. Xu, Y. Yang, J. Wang, and A. L. Yuille. Explain Images with Multimodal Recurrent Neural Networks. CoRR, abs/1410.1090, 2014.
...
- [40] M. Mitchell, J. Dodge, A. Goyal, K. Yamaguchi, K. Stratos, X. Han, A. Mensch, A. Berg, T. L. Berg, and H. Daume III. Midge: Generating Image Descriptions From Computer Vision Detections. In ACL, 2012.
...
- [52] R. Vendantam, X. Lin, T. Batra, C. L. Zitnick, and D. Parikh. Learning common sense through visual abstraction. In *ICCV*, 2015.
...
- [55] L. Yu, E. Park, A. C. Berg, and T. L. Berg. Visual madlibs: Fill-in-theblank description generation and question answering. In *ICCV*, 2015.
