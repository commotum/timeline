...
$$s_t = \mathcal{S}(s_{t-1}, W_x x_t) \tag{1}$$

$$y_t = W_y s_t + b_y \tag{2}$$
...
$$h_t^n = \sigma \left( W_h s_t^n + b_h \right) \tag{5}$$
...
$$R(t) = 1 - \sum_{n=1}^{N(t)-1} h_t^n \tag{8}$$

and  $\epsilon$  is a small constant (0.01 for the experiments in this paper), whose purpose is to allow computation to halt after a single update if  $h_t^1 >= 1 - \epsilon$ , as otherwise a minimum of two updates would be required for every input step. It follows directly from the definition that  $\sum_{n=1}^{N(t)} p_t^n = 1$  and  $0 \le p_t^n \le 1 \ \forall n$ , so this is a valid probability distribution. A similar distribution was recently used to define differentiable *push* and *pop* operations for neural stacks and queues [11].

At this point we could proceed stochastically by sampling  $\hat{n}$  from  $p_t^n$  and setting  $s_t = s_t^{\hat{n}}$ ,  $y^t = y_t^{\hat{n}}$ . However we will eschew sampling techniques and the associated problems of noisy gradients, instead using  $p_t^n$  to determine mean-field updates for the states and outputs:

$$s_t = \sum_{n=1}^{N(t)} p_t^n s_t^n \qquad y_t = \sum_{n=1}^{N(t)} p_t^n y_t^n \tag{9}$$
...
**Figure 1: RNN Computation Graph.** An RNN unrolled over two input steps (separated by vertical dotted lines). The input and output weights  $W_x, W_y$ , and the state transition operator  $\mathcal{S}$  are shared over all steps.
...
$$\mathcal{P}(\mathbf{x}) = \sum_{t=1}^{T} \rho_t \tag{11}$$

Since  $R(t) \in (0,1)$ ,  $\mathcal{P}(\mathbf{x})$  is an upper bound on the (non-differentiable) property we ultimately want to reduce, namely the total computation  $\sum_{t=1}^{T} N(t)$  during the sequence<sup>1</sup>. We can encourage the network to minimise  $\mathcal{P}(\mathbf{x})$  by modifying the sequence loss function  $\mathcal{L}(\mathbf{x}, \mathbf{y})$ 
...
$$N(t) = \min\{M, \min\{n' : \sum_{n=1}^{n'} h_t^n > 1 - \epsilon\}\}$$
(13)
...
$$\frac{\partial \mathcal{L}(\mathbf{x}, \mathbf{y})}{\partial p_t^n} = \frac{\partial \mathcal{L}(\mathbf{x}, \mathbf{y})}{\partial y_t} y_t^n + \frac{\partial \mathcal{L}(\mathbf{x}, \mathbf{y})}{\partial s_t} s_t^n$$
(17)

while, from Equations (6) and (8)

$$\frac{\partial p_t^{n'}}{\partial h_t^n} = \begin{cases}
\delta_{n,n'} & \text{if } n' < N(t) \text{ and } n < N(t) \\
-1 & \text{if } n' = N(t) \text{ and } n < N(t) \\
0 & \text{if } n = N(t)
\end{cases}$$
(18)
...
$$\frac{\partial \hat{\mathcal{L}}(\mathbf{x}, \mathbf{y})}{\partial h_t^n} = \frac{\partial \mathcal{L}(\mathbf{x}, \mathbf{y})}{\partial y_t} \left( y_t^n - y_t^{N(t)} \right) + \frac{\partial \mathcal{L}(\mathbf{x}, \mathbf{y})}{\partial s_t} \left( s_t^n - s_t^{N(t)} \right) - \tau \tag{19}$$

while for n = N(t)

$$\frac{\partial \hat{\mathcal{L}}(\mathbf{x}, \mathbf{y})}{\partial h_{+}^{N(t)}} = 0 \tag{20}$$
