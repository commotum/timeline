...
We initialize a policy  $\pi = \rho$ , and then fine-tune  $\pi$  to perform the task well using RL. If the task was defined by a reward function  $r: X \times Y \to \mathbb{R}$ , then we could use RL to directly optimize the expected reward:

$$\mathbb{E}_{\pi}\left[r\right] = \mathbb{E}_{x \sim \mathcal{D}, y \sim \pi(\cdot \mid x)}\left[r(x, y)\right]$$
...
For training the policy  $\pi$ , we use the PPO2 version of Proximal Policy Optimization from Dhariwal et al. (2017). We use 2M episodes (x,y) pairs),  $\gamma=1$ , four PPO epochs per batch with one minibatch each, and default values for the other parameters. We use batch size 1024 for style tasks and 512 for summarization. We do not use dropout for policy training. The learning rate was  $1.41 \times 10^{-5}$  for style tasks and  $7.07 \times 10^{-6}$  for summarization.

Models trained with different seeds and the same KL penalty  $\beta$  sometimes end up with quite different values of  $\mathrm{KL}(\pi,\rho)$ , making them hard to compare. To fix this, for some experiments we dynamically vary  $\beta$  to target a particular value of  $\mathrm{KL}(\pi,\rho)$  using the log-space proportional controller

$$e_t = \operatorname{clip}\left(\frac{\operatorname{KL}(\pi_t, \rho) - \operatorname{KL}_{\operatorname{target}}}{\operatorname{KL}_{\operatorname{target}}}, -0.2, 0.2\right)$$
...
<sup>&</sup>lt;sup>1</sup>In early experiments we found that it was hard for humans to provide consistent fine-grained quantitative distinctions when asked for an absolute number, and experiments on synthetic tasks confirmed that comparisons were almost as useful.
...
To study our method in a controlled setting, we first apply it to optimize a known reward function  $r_s$  designed to reflect some of the complexity of human judgments. We construct

 $r_s$  by training a classifier<sup>3</sup> on a binarized, balanced subsample of the Amazon review dataset of McAuley et al. (2015). The classifier predicts whether a review is positive or negative, and we define  $r_s(x,y)$  as the classifier's log odds that a review is positive (the input to the final sigmoid layer).
...
Because we know the reward function, we can also analytically compute the optimal policy and compare it to our learned policies. With a constraint on the KL divergence

<sup>&</sup>lt;sup>2</sup>Code at https://github.com/openai/lm-human-preferences.

<sup>&</sup>lt;sup>3</sup>The model is a Transformer with 6 layers, 8 attention heads, and embedding size 512.
...
|----------------------------|-------------|---|-----|-----------------|-----|--|
| 5k offline vs. zero-shot   | 88%         | 1 | 2%  | 86%             | 14% |  |
| 5k offline vs. mock        | <b>77</b> % | 2 | 23% |                 |     |  |
| 5k offline vs. 20k offline | 48%         | 5 | 2%  | 47%             | 53% |  |
| 5k offline vs. 5k online   | 50%         | 5 | 50% | 48%             | 52% |  |
...
<sup>&</sup>lt;sup>4</sup>This is a crude approximation for "end of sentence." We chose it because it is easy to integrate into the RL loop, and even a crude approximation is sufficient for the intended purpose of making the human evaluation task somewhat easier.

| context    | Pearl thought to herself that what they were about to do was exactly the sort of thing   |                                    |                             |  |  |  |
|------------|------------------------------------------------------------------------------------------|------------------------------------|-----------------------------|--|--|--|
|            | that they could do to help the                                                           | villagers. They were all terrified | d of these guys.            |  |  |  |
|            | At the police station the three                                                          | e walked up to the counter beh     | ind which was a senior      |  |  |  |
|            | constable studying some pape                                                             | rs.                                |                             |  |  |  |
|            | Continuation 1                                                                           | Continuation 2                     | Continuation 3              |  |  |  |
| zero-shot  | "Hello, I'm Pearl and this is  "May we speak to the police  'Hello, can I help you?'     |                                    |                             |  |  |  |
|            | my friend, Mike," said Pearl. officer, sir?" asked the one in Yes, we're the same people |                                    |                             |  |  |  |
|            | charge. that the people were talking                                                     |                                    |                             |  |  |  |
|            | about.                                                                                   |                                    |                             |  |  |  |
| 5k offline | He turned to them with a The constable stood up and He smiled at them and waved          |                                    |                             |  |  |  |
| fine-tune  | smile. "Good afternoon, smiled as he saw them, obvilethem in, his eyes twinkling as      |                                    |                             |  |  |  |
|            | ladies. I'm Detective Inspec-                                                            | ously pleased to see them.         | he listened to their tales. |  |  |  |
|            | tor Jones.                                                                               |                                    |                             |  |  |  |
...
|------------|-------------------------------------------------------------------|-------------------------------------------------------------|---------------------------------|--|--|
|            |                                                                   | e to be traveling here then? Ail                            |                                 |  |  |
|            | make sure they weren't being                                      | followed.                                                   |                                 |  |  |
|            | Continuation 1                                                    | Continuation 2                                              | Continuation 3                  |  |  |
| zero-shot  | There were no signs of any-                                       | "Because I have a friend that                               | It was hard to see that far. "I |  |  |
|            | one.                                                              | is in the area and he will be do not like that word.        |                                 |  |  |
|            | "It is safe enough," Ailios                                       | coming with us.                                             |                                 |  |  |
|            | said.                                                             |                                                             |                                 |  |  |
| 5k offline | Kaya crouched low, her eyes                                       | She put her hand on the                                     | She strode out the door         |  |  |
| fine-tune  | wide in the moonlight. Her                                        | Her sword strapped to her back, and walked down the street, |                                 |  |  |
|            | body was tense.                                                   | and then pulled it out.                                     | her nose wrinkled in disap-     |  |  |
|            |                                                                   |                                                             | proval.                         |  |  |
...
<sup>&</sup>lt;sup>5</sup>The descriptiveness policy trained with 2.5k samples performed poorly, but we believe this is due to randomness in RL.

|                            |        | TI    | .;DR   |        |        | CNN/D  | aily Mail |        |
|----------------------------|--------|-------|--------|--------|--------|--------|-----------|--------|
...
| 60k fine-tuned vs. 30k fine-tuned              | 40% | 60%   | <b>62%</b> | 38%            |
...
| 60k fine-tune | The Solar Impulse 2, the experimental plane attempting to fly around the world without    |
...
| 60k fine-tune | is attempting to fly around the world without using a drop of fuel. The team, which       |
|               | includes Bertrand Piccard, is taking turns flying the single-seater. The plane took off   |
|               | from Abu Dhabi on March 9 and has successfully flown through Oman.                        |
...
- Szymon Sidor, Yuhuai Wu, and Peter Zhokhov. Openai baselines. https://github.com/openai/baselines, 2017.
- Yang Gao, Christian M Meyer, and Iryna Gurevych. Preference-based interactive multi-document summarisation. *arXiv* preprint arXiv:1906.02923, 2019a.
...
- Sebastian Gehrmann, Zachary Ziegler, and Alexander Rush. Generating abstractive summaries with finetuned language models. In *TL;DR Challenge System Descriptions*, 2019.
...
- Borja Ibarz, Jan Leike, Tobias Pohlen, Geoffrey Irving, Shane Legg, and Dario Amodei. Reward learning from human preferences and demonstrations in Atari. In *Advances in Neural Information Processing Systems*, 2018. URL https://arxiv.org/abs/1811.06521.
...
- Wojciech Kryściński, Nitish Shirish Keskar, Bryan Mc-Cann, Caiming Xiong, and Richard Socher. Neural text summarization: A critical evaluation. *arXiv preprint arXiv:1908.08960*, 2019.
...
- Jelena Luketina, Nantas Nardelli, Gregory Farquhar, Jakob Foerster, Jacob Andreas, Edward Grefenstette, Shimon Whiteson, and Tim Rocktäschel. A survey of reinforcement learning informed by natural language. In *Proceed*ings of the Twenty-Eight International Joint Conference on Artificial Intelligence, IJCAI-19. International Joint Conferences on Artificial Intelligence Organization, 8 2019.
...
- Alec Radford, Jeff Wu, Rewon Child, David Luan, Dario Amodei, and Ilya Sutskever. Language models are unsupervised multitask learners, 2019. URL https://d4mucfpksywv.cloudfront.net/better-language-models/language\_models\_are\_unsupervised\_multitask\_learners.pdf.
...
| Between labelers              | $38{\pm}2\%$ | 46±2% |
| Between an author & a labeler | $44{\pm}5\%$ | 38±5% |
| Between authors               | $62 \pm 5\%$ | 61±5% |
...
| bonus        | easily easily easily!                                                                    | easily easily easily!                          |  |  |  |
...
| no penalty / | ! These These These sound flowed instantly                                               | ! These These These sound flowed instantly     |  |  |  |
| bonus        | easily easily easily!                                                                    | easily easily easily!                          |  |  |  |
| entropy      | initially relativelyprisingly instantlypris-                                             | 6- overwhelmingly Loganprisingly Liam in       |  |  |  |
| bonus        | ingly Liam Liam! Love this ride instantly                                                | stantly surprisingly! Love this ride instantly |  |  |  |
|              |                                                                                          | tightly                                        |  |  |  |
...
| bonus        | overwhelmingly! Love this ride instantly                                                 | Love this ride instantly instantly             |  |  |  |
|              | tightly                                                                                  | tightly                                        |  |  |  |
|              |                                                                                          |                                                |  |  |  |
...
|---------------|--------------------------------------------------------------------------------------|------------------------------------|--------------------------------|--|--|
|               |                                                                                      | n, who is now the chairman of t    |                                |  |  |
|               | Continuation 1                                                                       | Continuation 2                     | Continuation 3                 |  |  |
| zero-shot     | He was running for the                                                               | For weeks, I went around the       | He was one of the first teach- |  |  |
|               | supreme court and his cam-                                                           | county, telling everyone how       | ers to lead by example. It     |  |  |
|               | paign was violent. There was                                                         | amazing he was and how             | was incredibly inspiring to    |  |  |
|               | an incident at a press confer-                                                       | much I admired him.                | see.                           |  |  |
|               | ence.                                                                                |                                    |                                |  |  |
| mock          | I was really impressed with                                                          | He was a real hero to the peo-     | But he just sat around and     |  |  |
|               | how he handled the board,                                                            | ple in town. He was a real         | watched. I love that story.    |  |  |
|               | and I thought it was a great                                                         | hero.                              | You should read it again.      |  |  |
|               | story.                                                                               |                                    |                                |  |  |
| 5k offline    | Paul Gibson is a very intelli-                                                       | Paul is a very nice guy, a true    | He was a good man, with a      |  |  |
| fine-tune     | gent man. He's a good heart                                                          | gentleman, and we all know         | great smile, a great sense of  |  |  |
|               | man. He has a good sense of                                                          | he's a really cute guy.            | humor, a great family.         |  |  |
|               | humor.                                                                               |                                    |                                |  |  |
| context       |                                                                                      | hat they were about to do was      |                                |  |  |
|               |                                                                                      | villagers. They were all terrified |                                |  |  |
|               | =                                                                                    | e walked up to the counter beh     | ind which was a senior         |  |  |
|               | constable studying some pape                                                         |                                    |                                |  |  |
|               | Continuation 1                                                                       | Continuation 2                     | Continuation 3                 |  |  |
| zero-shot     | "Hello, I'm Pearl and this is                                                        | "May we speak to the police        | 'Hello, can I help you?'       |  |  |
|               | my friend, Mike," said Pearl.                                                        | officer, sir?" asked the one in    | 'Yes, we're the same people    |  |  |
|               |                                                                                      | charge.                            | that the people were talking   |  |  |
|               |                                                                                      |                                    | about.                         |  |  |
| mock          | "I'm Detective Sergeant                                                              | He looked up at the three          | "We have a friend who is in    |  |  |
|               | Lewis, this is Detective                                                             | with a friendly smile.             | trouble. He was kidnapped      |  |  |
|               | Sergeant Jones and this is                                                           | "Good day, gentlemen.              | by these guys," the boy said.  |  |  |
|               | Detective Sergeant Porter.                                                           |                                    |                                |  |  |
| 5k offline    | He turned to them with a                                                             | The constable stood up and         | He smiled at them and waved    |  |  |
| fine-tune     | smile. "Good afternoon,                                                              | smiled as he saw them, obvi-       | them in, his eyes twinkling as |  |  |
|               | ladies. I'm Detective Inspec-                                                        | ously pleased to see them.         | he listened to their tales.    |  |  |
|               | tor Jones.                                                                           |                                    |                                |  |  |
| context       |                                                                                      | coming, with two hearths that      | • •                            |  |  |
|               |                                                                                      | at Gunnar's feet. Ailia looked u   | up at Gunnar and rose to       |  |  |
|               | her feet as calmly as possible.                                                      |                                    |                                |  |  |
|               | Continuation 1                                                                       | Continuation 2                     | Continuation 3                 |  |  |
| zero-shot     | "I will not fight."                                                                  | "I'll give you one last chance     | "I'm sorry. I didn't mean      |  |  |
|               | "You will fight for us," Gun-                                                        | to tell me where that is,"         | to"                            |  |  |
|               | nar said.                                                                            | Bjorn said.                        | "Give me your hand." Gun-      |  |  |
|               | WT1 41 T1 41 T                                                                       | UT 1 1                             | nar told her.                  |  |  |
| mock          | "I love this. I love this whole                                                      | "I am very pleased to meet         | "Bravely done, Gunnar.         |  |  |
|               | room. I'm going to enjoy it.                                                         | you, and let me assure you,        | Thank you." Ailia smiled at    |  |  |
| <b>71</b> 00. |                                                                                      | you will not regret this.          | him.                           |  |  |
| 5k offline    | Gunnar smiled at Ailia and                                                           | "You've been a true friend         | Gunnar was smiling at her      |  |  |
| fine-tune     | waited for her to smile back.                                                        | to my son, Gunnar. Thank           | and his pretty, blue eyes were |  |  |
|               | She was smiling, she knew.                                                           | you." She smiled.                  | filled with contentment.       |  |  |
...
|            | said.                                                                                  |                                                                 |                                 |  |  |
| 5k offline | Kaya crouched low, her eyes                                                            | She put her hand on the                                         | She strode out the door         |  |  |
| fine-tune  | wide in the moonlight. Her                                                             | sword strapped to her back,                                     | and walked down the street,     |  |  |
|            | body was tense.                                                                        | and then pulled it out.                                         | her nose wrinkled in disap-     |  |  |
|            |                                                                                        |                                                                 | proval.                         |  |  |
| context    | 1 *                                                                                    | it were full of girls, it were, all                             | •                               |  |  |
|            |                                                                                        | d work, really I don't. Mama sa                                 | -                               |  |  |
|            | Continuation 1                                                                         | Continuation 2                                                  | Continuation 3                  |  |  |
| zero-shot  | But the girls keep on sneakin'                                                         | But in the end, it weren't any-                                 | I was just lucky to get this    |  |  |
|            | in 'em. An' one day, one of                                                            | thing like she'd promised me.                                   | job. But as I stayed there, I   |  |  |
|            | em found me.                                                                           | Some of them were worse'n                                       | started to see things.          |  |  |
|            |                                                                                        | me.                                                             |                                 |  |  |
| 5k offline | But" The girl hiccupped,                                                               | But then" His voice trailed                                     | She said I could keep my        |  |  |
| fine-tune  | then pressed her lips together                                                         | off and he shook his head, his                                  | eyes open and my head clear     |  |  |
|            | and shook her head.                                                                    | eyes going distant, somber.                                     | and make a good target.         |  |  |
| context    | _                                                                                      | up and shifted closer to him until she was kneeling behind him. |                                 |  |  |
|            |                                                                                        | aid a splayed hand on his bare b                                |                                 |  |  |
|            |                                                                                        | her words. "I will never lie to                                 |                                 |  |  |
|            | Continuation 1                                                                         | Continuation 2                                                  | Continuation 3                  |  |  |
| zero-shot  | I love you."                                                                           | You have to know that."                                         | Whatever happens, I'll tell     |  |  |
|            | TT . 1 . 1 . 1 . 1                                                                     | UT 1 '11                                                        | you the truth."                 |  |  |
|            | He turned to her, his                                                                  | "I know you will never                                          |                                 |  |  |
| 5k offline | face a mask of pain.  I promise."                                                      | lie to me again.  I swear it."                                  | "I'm sorry.                     |  |  |
| fine-tune  | i promise.                                                                             | 1 Sweaf II.                                                     | I love you."                    |  |  |
| mie-tune   | His hands curled around her                                                            | He exhaled heavily and                                          | Max leaned his head             |  |  |
|            | forearms, his fingers digging                                                          | hugged her close, pressing                                      | back and closed his eyes.       |  |  |
|            | deep into her flesh.                                                                   | his lips to her forehead.                                       | His breathing changed.          |  |  |
|            | Geep into her nesh.                                                                    | ms ups to nei foreneau.                                         | The oreathing changeu.          |  |  |
...
| context post               | Okay, this was actually going to go into a comment on youtube, but Emma said she wanted more traffic on reddit.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
|----------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
| reference<br>summary       | Emma music gud yes very love magic                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| zero-shot                  | I'm a musician myself, have been for many years now. And Emma Blackery is my favourite artist.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| supervised + 60k fine-tune | Emma Blackery is my favourite artist, and I love her music to death. I wish I could listen to her music all the time. However, I feel like her music destroys me emotionally. I feel like I need to enjoy her music for the human part of it, and not the technical part. She's my favourite artist.                                                                                                                                                                                                                                                                                                                                                |
|                            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| context post  | It really does. I did the first 5 seasons in a couple months, and ultimately I enjoyed       |
|---------------|----------------------------------------------------------------------------------------------|
|               | them. The first three seasons cover just about all the really good stuff. Russell            |
...
|               | half-hollow/half-warm feeling you should when you finish a series of books or a long         |
...
| zero-shot     | I finished S6 and still feel like I'm in the same place as I was with S5.                    |
|               | Favorite Episodes:                                                                           |
|               | S1: "Tough Love"                                                                             |
|               | S2: "Crossover"                                                                              |
|               | S3: "The Party"                                                                              |
| 60k fine-tune | I did the first 5 seasons in a couple months, and ultimately I enjoyed them. The first       |
...
| 60k fine-tune | three seasons is frustrating because it's so similar to the first three seasons, and it's so |
...
| reference<br>summary | neither you nor I have any idea what the actual problem is so there is no basis for saying the problem favors either candidate at this time, or if either candidate would even be affected.                                                                                                                                                                                           |
| zero-shot            | Election officials did not count ballots in a timely manner, by hand, to meet the threshold of getting votes counted on election night.                                                                                                                                                                                                                                               |
...
| 60k fine-tune        | The machine shows a tally of 120 votes having been processed on it. The actual reported vote tally to the election board was 100 votes. The 20 vote differential on the machine is due to 20 times a ballot needing to be re-scanned due to error. No double-counting of votes occurs but there is also a delta between what the machine says                                         |
| supervised           | Stop assuming the election was completely free of double-counting before you even begin to discuss the actual results. For any result to be close, it must be based on the                                                                                                                                                                                                            |
|                      | total vote totals reported to the election board and not just the machine results. The election board did not report the machine results to the election board.                                                                                                                                                                                                                       |
| supervised +         | The reported vote tallies may have been factually correct but there is no way to validate                                                                                                                                                                                                                                                                                             |
| 60k fine-tune        | them against the machine record. The machine may have actually been accurate and the                                                                                                                                                                                                                                                                                                  |
|                      | machine may have been faulty in other ways. The election may have been considered fair but the machine may not have been. The results are still in dispute.                                                                                                                                                                                                                           |
...
| context arti-<br>cle | Google launched a new U.S. wireless service today that switches between Wi-Fi and cellular networks                                                                                                                                                                                                |
|----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
| 60k fine-tune        | Google is already the world's most popular phone software provider, and a pay-TV operator - and now it wants to be your mobile network carrier.                                                                                                                                                    |
|                      | The company has unveiled a U.S. wireless service that switches between Wi-Fi and cellular networks to curb data use and keep phone bills low.                                                                                                                                                      |
| supervised           | Google launched a new U.S. wireless service today. Called 'Project Fi,' it switches between Wi-Fi and cellular networks. The service, called 'Project Fi,' debuted today.                                                                                                                          |
|                      | About two months after Google announced its plans to expand its ever-growing empire                                                                                                                                                                                                                |
| supervised +         | into providing wireless connections for smartphones.  Google launched a new U.S. wireless service today that switches between Wi-Fi and                                                                                                                                                            |
| 60k fine-tune        | cellular networks. The service, called 'Project Fi,' debuted today, about two months after                                                                                                                                                                                                         |
| Jon mile tune        | Google revealed its plans to expand its ever-growing empire into providing wireless connections for smartphones.                                                                                                                                                                                   |
| 6. Dandam (T         | 0.5) summaries for our CNN/DM summaries to a took with contexts (shown two cots)                                                                                                                                                                                                                   |
...
|                      | With the date of their breaks artisinated boot loss than a growth array Florid                                                                                                                             |
|----------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| context arti-<br>cle | With the date of their hugely anticipated bout less than a month away, Floyd Mayweather and Manny Pacquiao's training intensity has hit gruelling levels ahead of the clash in Las Vegas.                  |
|                      |                                                                                                                                                                                                            |
...
| reference            | Manny Pacquiao took time out from training to meet Mark Wahlberg. Floyd May-                                                                                                                               |
| summary              | weather shared a picture of him holding a weight with his head. The due meet in Las Vegas in a £160million encounter on May 2.                                                                             |
| zero-shot            | Mayweather and Pacquiao are going to clash in Las Vegas on May 2.                                                                                                                                          |
| 60k fine-tune        | With the date of their hugely anticipated bout less than a month away, Floyd Mayweather and Manny Pacquiao's training intensity has hit gruelling levels ahead of the clash in Las Vegas.                  |
|                      | Mayweather is as renowned for showing off his extravagant lifestyle as he is for going undefeated in his professional career.                                                                              |
...
| 60k fine-tune        | released two pictures from his training camp appearing hold a weight from his head.                                                                                                                        |
...
| 7. Random (T -       | 0.5) summaries for our CNN/DM summarization task, with contexts (shown truncated) ra                                                                                                                       |
...
| context arti-              | The Solar Impulse 2, the experimental plane attempting to fly around the world without using a drop of fuel, has been grounded by the weather in China.                                                                                                                                                                       |
|----------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|                            | asing a ground of the would in china.                                                                                                                                                                                                                                                                                         |
...
| reference<br>summary       | Solar plane attempting to be first to circumnavigate world without using fuel is stuck in China. Solar Impulse 2 attempts to prove the power of renewable energy.                                                                                                                                                             |
| zero-shot                  | The plane has been grounded in China for two weeks because of bad weather.                                                                                                                                                                                                                                                    |
| 60k fine-tune              | The Solar Impulse 2, the experimental plane attempting to fly around the world without using a drop of fuel, has been grounded by the weather in China.                                                                                                                                                                       |
|                            | What was supposed to be an overnight pit stop in the southwestern city of Chongqing has now stretched into a two-and-a-half week stay.                                                                                                                                                                                        |
