...
<sup>\*</sup>Equal contribution <sup>1</sup>OpenAI, San Francisco. Correspondence to: <jukebox@openai.com>.

<sup>&</sup>lt;sup>1</sup>Richard Feynmann famously said, "What I cannot create, I do not understand"
...
To make this task feasible, we use the VQ-VAE (Oord et al., 2017; Dieleman et al., 2018; Razavi et al., 2019) to compress raw audio to a lower-dimensional space. A one-dimensional VQ-VAE learns to encode an input sequence  $\mathbf{x} = \langle \mathbf{x}_t \rangle_{t=1}^T$  using a sequence of discrete tokens  $\mathbf{z} = \langle z_s \in [K] \rangle_{s=1}^S$ , where K denotes the vocabulary size and we call the ratio T/S the hop length. It consists of an encoder  $E(\mathbf{x})$  which encodes  $\mathbf{x}$  into a sequence of latent vectors  $\mathbf{h} = \langle \mathbf{h}_s \rangle_{s=1}^S$ , a bottleneck that quantizes  $\mathbf{h}_s \mapsto \mathbf{e}_{z_s}$  by mapping each  $\mathbf{h}_s$  to its nearest vector  $\mathbf{e}_{z_s}$  from a codebook  $C = \{\mathbf{e}_k\}_{k=1}^K$ , and a decoder  $D(\mathbf{e})$  that decodes the embedding vectors
...
$$\mathcal{L} = \mathcal{L}_{recons} + \mathcal{L}_{codebook} + \beta \mathcal{L}_{commit}$$
 (1)

$$\mathcal{L}_{\text{recons}} = \frac{1}{T} \sum_{t} \|\mathbf{x}_t - D(\mathbf{e}_{z_t})\|_2^2$$
 (2)

$$\mathcal{L}_{\text{codebook}} = \frac{1}{S} \sum_{s} \|\text{sg}\left[\mathbf{h}_{s}\right] - \mathbf{e}_{z_{s}}\|_{2}^{2}$$
 (3)

$$\mathcal{L}_{\text{commit}} = \frac{1}{S} \sum_{s} \|\mathbf{h}_{s} - \operatorname{sg}\left[\mathbf{e}_{z_{s}}\right]\|_{2}^{2}$$
 (4)

where sg denotes the stop-gradient operation, which passes zero gradient during backpropagation. The reconstruction loss  $\mathcal{L}_{recons}$  penalizes for the distance between the input x and the reconstructed output  $\hat{\mathbf{x}} = D(\mathbf{e}_z)$ , and  $\mathcal{L}_{\text{codebook}}$  penalizes the codebook for the distance between the encodings h and their nearest neighbors  $e_z$  from the codebook. To stabilize the encoder, we also add  $\mathcal{L}_{commit}$  to prevent the encodings from fluctuating too much, where the weight  $\beta$ controls the amount of contribution of this loss. To speed up training, the codebook loss  $\mathcal{L}_{codebook}$  instead uses EMA updates over the codebook variables. Razavi et al. (2019) extends this to a hierarchical model where they train a single encoder and decoder but break up the latent sequence  $\mathbf{h}$  into a multi-level representation  $[\mathbf{h}^{(1)}, \cdots, \mathbf{h}^{(L)}]$  with decreasing sequence lengths, each learning its own codebook  $C^{(l)}$ . They use non-autoregressive encoder-decoders and jointly train all levels with a simple mean-squared loss.

## 3. Music VQ-VAE

Inspired by the results from the hierarchical VQ-VAE model (Razavi et al., 2019) for images, we consider applying the same technique to model raw audio using three different levels of abstraction, as illustrated in Figure 1. At each level, we use residual networks consisting of WaveNet-style non-causal 1-D dilated convolutions, interleaved with downsampling and upsampling 1-D convolutions to match different hop lengths. A detailed description of the architecture is provided in Appendix B.1. We make a number of modifications to our VQ-VAE compared to the ones in (Oord et al., 2017; Razavi et al., 2019), as described in the following subsections.
...
$$\mathcal{L}_{spec} = \||STFT(\mathbf{x})| - |STFT(\widehat{\mathbf{x}})|\|_{2}$$
...
we use the sum of the spectral losses  $\mathcal{L}_{spec}$  calculated over multiple STFT parameters that trade-off time and frequency resolutions (Yamamoto et al., 2020).
...
<sup>&</sup>lt;sup>2</sup>The gradient also needs to break symmetry with the encoder output features, which is the case here since the weights of the input projections in the attention are not zero.
...
The final model, which we call Jukebox, uses all these improvements. Because everyone experiences music differently, it is generally tricky and not very meaningful to evaluate samples by the mean opinion score or FID-like metrics. We manually evaluate coherence, musicality, diver-
...
- Arık, S. Ö., Chen, J., Peng, K., Ping, W., and Zhou, Y. Neural voice cloning with a few samples. In *Advances in Neural Information Processing Systems*, pp. 10019–10029. 2018a.
...
- Huang, C. A., Cooijmans, T., Roberts, A., Courville, A. C., and Eck, D. Counterpoint by convolution. In *Interna*tional Society for Music Information Retrieval Conference, pp. 211–218, 2017.
...
- Oord, A. v. d., Li, Y., Babuschkin, I., Simonyan, K., Vinyals,
  ...
  D. Parallel WaveNet: Fast high-fidelity speech synthesis.
  In *International Conference on Machine Learning*, pp. 3918–3926, 2018.
...
- Ping, W., Peng, K., Gibiansky, A., Arik, S. O., Kannan,
  ...
  3: 2000-speaker neural text-to-speech. In *International Conference on Learning Representations*, 2018.
...
- Yamamoto, R., Song, E., and Kim, J.-M. Parallel Wave-GAN: A fast waveform generation model based on generative adversarial networks with multi-resolution spectrogram. In *International Conference on Acoustics, Speech, and Signal Processing*, 2020.
- Yang, L., Chou, S., and Yang, Y. Midinet: A convolutional generative adversarial network for symbolic-domain music generation. In *International Society for Music Infor*mation Retrieval Conference, pp. 324–331, 2017.
...
| Commit weight $\beta$               | 0.02            |
| Codebook EMA $\gamma$               | 0.99            |
| Codebook size                       | 2048            |
