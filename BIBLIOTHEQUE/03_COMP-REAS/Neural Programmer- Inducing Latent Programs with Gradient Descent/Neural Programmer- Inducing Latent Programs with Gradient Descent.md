...
Ilva Sutskever Google Brain ilyasu@google.com
...
where  $V(w_i) \in \mathbb{R}^d$  represents the embedded representation of the word  $w_i$ ,  $[a;b] \in \mathbb{R}^{2d}$  represents the concatenation of two vectors  $a,b \in \mathbb{R}^d$ ,  $W^{question} \in \mathbb{R}^{d \times 2d}$  is the recurrent matrix of the question RNN, tanh is the element-wise non-linearity function and  $z_Q \in \mathbb{R}^d$  is the representation of the question. We set  $z_0$  to  $[0]^d$ . We pre-process the question by removing numbers from it and storing the numbers in a separate list. Along with the numbers we store the word that appeared to the left of it in the question which is useful to compute the pivot values for the comparison operations described in Section 2.3.
...
The selector produces two probability distributions at every time step t  $(t=1,2,\ldots,T)$ : one probability distribution over the set of operations and another probability distribution over the set of columns. The inputs to the selector are the question representation  $(q \in \mathbb{R}^d)$  from the question module and the output of the history RNN (described in Section 2.4) at time step t  $(h_t \in \mathbb{R}^d)$  which stores information about the operations and columns selected by the model up to the previous step.
...
<sup>&</sup>lt;sup>1</sup>It is trivial to extend the model to support general text responses by adding a decoder RNN to generate text sentences.
...
| Comparison    | Lesser     | $  l_t[i][j] = table[i][j] < pivot_l, \forall (i, j), i = 1,, M, j = 1,, C$                |
...
| Assign Lookup | assign     | $assign_t[i][j] = row\_select_{t-1}[i], \forall (i,j)i = 1, 2,, M, j = 1, 2,, C$           |
| Reset         | Reset      | $reset_t[i] = 1, \forall i = 1, 2, \dots, M$                                               |

Table 1: List of operations along with their definitions at time step t,  $table \in \mathbb{R}^{M \times C}$  is the data source in the form of a table and  $row\_select_t \in [0,1]^M$  functions as a row selector.

While the definitions of the operations are fairly straightforward, comparison operations greater and lesser require a pivot value as input (refer Table 1), which appears in the question. Let  $qn_1, qn_2, \ldots, qn_N$  be the numbers that appear in the question.
...
$$pivot_{op} = \sum_{i=1}^{N} \beta_{op}(i)qn_{i}$$

where  $U(op) \in \mathbb{R}^d$  is the vector representation of operation  $op\ (op \in \{\text{greater}, \text{lesser}\})$  and  $Z \in \mathbb{R}^{N \times d}$  is the matrix storing the hidden vectors of the question RNN at positions to the left of the occurrence of the numbers.

By overloading the definition of  $\alpha_t^{op}$  and  $\alpha_t^{col}$ , let  $\alpha_t^{op}(x)$  and  $\alpha_t^{col}(j)$  denote the probability assigned by the selector to operation x ( $x \in \{\text{sum, count, difference, greater, lesser, and, or, assign, reset}\}$ ) and column j ( $\forall j = 1, 2, \ldots, C$ ) at time step t respectively.
...
<sup>&</sup>lt;sup>2</sup>This choice is made to reflect the common case in English where the pivot number is usually mentioned after the operation but it is trivial to extend to use hidden vectors both in the left and the right of the number.
...
$$lookup\_answer_t[i][j] = \alpha_t^{col}(j)\alpha_t^{op}(assign)assign_t[i][j], \forall (i,j)i = 1, 2, \dots, M, j = 1, 2, \dots, C$$

The row selector variable is given by:

$$\begin{aligned} row\_select_t[i] &= \alpha_t^{op}(\text{and}) and_t[i] + \alpha_t^{op}(\text{or}) or_t[i] + \alpha_t^{op}(\text{reset}) reset_t[i] + \\ &\sum_{t=0}^{C} \alpha_t^{col}(j) (\alpha_t^{op}(\text{greater}) g_t[i][j] + \alpha_t^{op}(\text{lesser}) l_t[i][j]), \forall i = 1, \dots, M \end{aligned}$$
...
$$D[k][p] = \frac{1}{M} \sum_{m=1}^{M} (B[m][k] \cdot A[m][k][p]) \quad \forall (k, p) \ k = 1, \dots, K, p = 1, \dots, d$$
...
$$text\_match[m][k] = sigmoid\left(\sum_{p=1}^{d} A[m][k][p] \cdot G[k][p]\right) \ \forall (m,k) \ m=1,\ldots,M, k=1,\ldots,K$$

Without loss of generality, let the first K ( $K \in [0, 1, ..., C]$ ) columns out of C columns of the table contain text entries while the remaining contain numeric entries. The row selector variable now is given by:

$$\begin{split} row\_select_t[i] &= \alpha_t^{op}(\text{and}) \, and_t[i] + \alpha_t^{op}(\text{or}) \, or_t[i] + \alpha_t^{op}(\text{reset}) \, reset_t[i] + \\ &\sum_{j=K+1}^{C} \alpha_t^{col}(j) (\alpha_t^{op}(\text{greater}) g_t[i][j] + \alpha_t^{op}(\text{lesser}) l_t[i][j]) + \\ &\sum_{j=1}^{K} \alpha_t^{col}(j) (\alpha_t^{op}(\text{text\_match}) \, text\_match_t[i][j], \forall i = 1, \dots, M \end{split}$$
...
$$\begin{split} L_{lookup}(lookup\_answer_T, y) &= -\frac{1}{MC} \sum_{i=1}^{M} \sum_{j=1}^{C} \bigg( y[i, j] \log(lookup\_answer_T[i, j]) + \\ & (1 - y[i, j]) \log(1 - lookup\_answer_T[i, j]) \bigg) \end{split}$$
...
- 4: **Question Module**: Run question RNN on the preprocessed question to get question representation q and list of hidden states  $z_1, z_2, \ldots, z_Q$
...
Many Columns: We increase the difficulty by experimenting with multiple columns (max\_columns = 3, 5 or 10). During training, the number of columns is randomly sampled from (1, max\_columns) and at test time every question had the maximum number of columns used during training.
...
and allows it to actively explore more programs. We use a schedule inspired from Welling & Teh (2011), where at every step we sample a Gaussian of 0 mean and variance= curr\_step $^{-0.55}$ .
...
| Туре                          | No. of Test Question Templates | Accuracy | % seen test |  |
|-------------------------------|--------------------------------|----------|-------------|--|
...
| Question                                                                             |   | Selected | Selected | $pivot_g$ | $pivot_l$ | Row       |
|--------------------------------------------------------------------------------------|---|----------|----------|-----------|-----------|-----------|
|                                                                                      |   | Op       | Column   |           |           | select    |
| greater 50.32 C and lesser 20.21 E sum H                                             |   | Greater  | С        |           |           | $g_1$     |
| What is the sum of numbers in column H whose field in column C is greater than 50.32 |   | Lesser   | Е        | 50.32     | 20.21     | $l_2$     |
|                                                                                      |   | And      | -        |           |           | $and_3$   |
| and field in Column E is lesser than 20.21.                                          | 4 | Sum      | Н        |           |           | $[0]^{M}$ |
| lesser -80.97 D or greater 12.57 B print F                                           | 1 | Lesser   | D        |           |           | $l_1$     |
| Print elements in column F                                                           | 2 | Greater  | В        | 12.57     | -80.97    | $g_2$     |
| whose field in column D is lesser than -80.97                                        |   | Or       | -        | 12.37     | -80.97    | $or_3$    |
...
| between sum of elements in                                                           |   | Count    | -        | -1        | -1        | $[0]^{M}$ |
...
- Hermann, Karl Moritz, Kociský, Tomás, Grefenstette, Edward, Espeholt, Lasse, Kay, Will, Suleyman, Mustafa, and Blunsom, Phil. Teaching machines to read and comprehend. *NIPS*, 2015.
...
- Shang, Lifeng, Lu, Zhengdogn, and Li, Hang. Neural responding machine for short-text conversation. *arXiv* preprint arXiv:1503.02364, 2015.
