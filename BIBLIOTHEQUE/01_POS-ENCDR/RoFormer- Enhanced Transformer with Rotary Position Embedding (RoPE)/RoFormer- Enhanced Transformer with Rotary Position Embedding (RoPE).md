...
**Keywords** Pre-training · Natural Language Models · Position Information Encoding · Pre-training · Natural Language Processing.
...
<sup>&</sup>lt;sup>1</sup>A stack of multiple CNN layers can also capture longer intra-token relation, here we only consider single layer setting.
...
$$f_{t:t\in\{q,k,v\}}(\boldsymbol{x}_i,i) := \boldsymbol{W}_{t:t\in\{q,k,v\}}(\boldsymbol{x}_i + \boldsymbol{p}_i), \tag{3}$$
...
$$R_{q}(\mathbf{x}_{q}, m)R_{k}(\mathbf{x}_{k}, m) = R_{q}(\mathbf{x}_{q}, \mathbf{x}_{k}, 0) = R_{q}(\mathbf{x}_{q}, 0)R_{k}(\mathbf{x}_{k}, 0) = ||\mathbf{q}|| ||\mathbf{k}||,$$
(26a)
...
$$q = f_q(\boldsymbol{x}_m, 0) = \boldsymbol{W}_q \boldsymbol{x}_n,$$
...
Taking the advantage of the sparsity of  $\mathbf{R}_{\Theta,m}^d$  in Equation (15), a more computational efficient realization of a multiplication of  $R_{\Theta}^d$  and  $\mathbf{x} \in \mathbb{R}^d$  is:
...
<sup>&</sup>lt;sup>2</sup>For this experiment, we adopt code (MIT License) from https://github.com/lucidrains/performer-pytorch
