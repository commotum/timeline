...
| Preetum Nakkiran*  | Gal Kaplun <sup>†</sup> | Yamini Bansal <sup>†</sup> | Tristan Yang       |
|--------------------|-------------------------|----------------------------|--------------------|
| Harvard University | Harvard University      | Harvard University         | Harvard University |

Boaz Barak Ilya Sutskever Harvard University OpenAI
...
<sup>\*</sup>Work performed in part while Preetum Nakkiran was interning at OpenAI, with Ilya Sutskever. We especially thank Mikhail Belkin and Christopher Olah for helpful discussions throughout this work. Correspondence Email: preetum@cs.harvard.edu
...
$$\mathrm{EMC}_{\mathcal{D},\epsilon}(\mathcal{T}) := \max \{ n \mid \mathbb{E}_{S \sim \mathcal{D}^n}[\mathrm{Error}_S(\mathcal{T}(S))] \leq \epsilon \}$$

where  $\operatorname{Error}_S(M)$  is the mean error of model M on train samples S.
...
- Mauro Cettolo, Christian Girardi, and Marcello Federico. Wit<sup>3</sup>: Web inventory of transcribed and translated talks. In *Proceedings of the 16<sup>th</sup> Conference of the European Association for Machine Translation (EAMT)*, pp. 261–268, Trento, Italy, May 2012.
...
| CIFAR 10        | CNN          | SGD        | <b>√</b>     | 0              | Х            | Х            | 5, 27      |
|                 |              |            | $\checkmark$ | 10             | $\checkmark$ | $\checkmark$ | 5, 27, 6   |
|                 |              |            | $\checkmark$ | 20             | $\checkmark$ | $\checkmark$ | 5, 27      |
|                 |              |            |              | 0              | X            | Х            | 5, 25      |
|                 |              |            |              | 10             | $\checkmark$ | $\checkmark$ | 5          |
|                 |              |            |              | 20             | $\checkmark$ | $\checkmark$ | 5          |
|                 |              | SGD + w.d. | $\checkmark$ | 20             | $\checkmark$ | $\checkmark$ | 21         |
|                 |              | Adam       |              | 0              | $\checkmark$ | _            | 25         |
|                 | ResNet       | Adam       | $\checkmark$ | 0              | X            | X            | 4, 10      |
|                 |              |            | $\checkmark$ | 5              | $\checkmark$ | _            | 4          |
|                 |              |            | $\checkmark$ | 10             | $\checkmark$ | $\checkmark$ | 4, 10      |
|                 |              |            | $\checkmark$ | 15             | $\checkmark$ | $\checkmark$ | 4, 2       |
|                 |              |            | $\checkmark$ | 20             | $\checkmark$ | $\checkmark$ | 4, 9, 10   |
|                 |              | Various    | $\checkmark$ | 20             | _            | $\checkmark$ | 16, 17, 18 |
| (subsampled)    | CNN          | SGD        | $\checkmark$ | 10             | $\checkmark$ | _            | 11a        |
|                 |              | SGD        | $\checkmark$ | 20             | $\checkmark$ | -            | 11a, 12    |
| (adversarial)   | ResNet       | SGD        |              | 0              | Robust err.  | _            | 26         |
| CIFAR 100       | ResNet       | Adam       | $\checkmark$ | 0              | $\checkmark$ | Х            | 4, 19, 10  |
|                 |              |            | $\checkmark$ | 10             | $\checkmark$ | $\checkmark$ | 4, 10      |
|                 |              |            | $\checkmark$ | 20             | $\checkmark$ | $\checkmark$ | 4, 10      |
|                 | CNN          | SGD        |              | 0              | $\checkmark$ | X            | 20         |
| IWSLT '14 de-en | Transformer  | Adam       |              | 0              | ✓            | X            | 8, 24      |
| (subsampled)    | Transformer  | Adam       |              | 0              | $\checkmark$ | X            | 11b, 23    |
| WMT '14 en-fr   | Transformer  | Adam       |              | 0              | $\checkmark$ | X            | 8, 24      |
...
**Transformers.** We consider the encoder-decoder Transformer model from Vaswani et al. (2017) with 6 layers and 8 attention heads per layer, as implemented by fairseq Ott et al. (2019). We scale the size of the network by modifying the embedding dimension ( $d_{\rm model}$ ), and scale the width of the fully-connected layers proportionally ( $d_{\rm ff}=4d_{\rm model}$ ). We train with 10% label smoothing and no drop-out, for 80 gradient steps.
...
$$(x, y, i) : x \in V_{src}^*, y \in V_{tat}^*, i \in \{0, \dots, |y|\}$$
...
$$\operatorname{Error}_{S}(M) = \frac{1}{|S|} \sum_{(x,y,i) \in S} -\log p_{M}(y_{i}|y_{< i}, x).$$
