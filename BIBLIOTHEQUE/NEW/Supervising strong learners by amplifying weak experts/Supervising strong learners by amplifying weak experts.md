...
Paul Christiano OpenAI paul@openai.com Buck Shlegeris \* bshlegeris@gmail.com
...
|------------------------|---------------------------|-------------------------------------------------------------------------------------------------|--------------------------|--|
| Supervised learning    | Learning data structures  | Image classification  00000000  /                      2222223  333333333  44444444  5555555555 | Long-term prediction     |  |
| Reinforcement learning | Playing games  O  X  X  X | Driving "well"                                                                                  | Designing transit system |  |
...
We propose a new method, Iterated Amplification, for a human expert H to train an ML agent X. Rather than having H demonstrate or evaluate the target behavior on their own, we allow them to invoke several copies of the current agent X to help them. We write  $\operatorname{Amplify^H}(X)$  for the composite system, consisting of H and several copies of X working together to solve a problem. The agent X then learns from  $\operatorname{Amplify^H}(X)$  in the same way that it would traditionally learn from H alone.
...
<sup>&</sup>lt;sup>2</sup>"When a measure becomes a target, it ceases to be a good measure"

- How do we construct  $\operatorname{Amplify^H}(X)$ ? In this article, we focus on delegation:  $\operatorname{Amplify^H}(X)$  answers a question Q by having H identify a sequence of useful subquestions, using X to compute a subanswer to each subquestion, and having H decide how to answer Q after seeing the subanswers.
- How does X learn from  $\operatorname{Amplify}^H(X)$ ? In this article, we focus on supervised learning: X is an autoregressive model trained to predict  $\operatorname{Amplify}^H(X)$ 's output. Future work could instead use imitation learning, or use  $\operatorname{Amplify}^H(X)$  to define a reward function that X maximizes with RL.

Initially X behaves randomly, so  $\operatorname{Amplify}^H(X)$  is essentially equivalent to H and we are effectively learning from an expert. Over time the agent X becomes more powerful and the role of the expert transitions into "coordinating" several copies of X to solve the problem better than a single copy could solve it. (Once X is very sophisticated, even tasks like "identify a useful subquestion" might be delegated.) As long as it is possible for multiple agents to collaboratively solve problems more effectively than a single agent (perhaps using human expertise to coordinate their efforts), then  $\operatorname{Amplify}^H(X)$  can outperform X and hence provide a useful training signal. We discuss this assumption in Section 5.
...
1. We repeatedly sample a question  $Q \sim \mathcal{D}$ , use  $\operatorname{Amplify}^H(X)$  to answer that question, and record every decision made by H during the process. That is, H finds a subquestion  $Q_1$  that would help them answer Q, and we compute the answer  $A_1 = X(Q_1)$ . We repeat this process k times, where k is a fixed parameter, and then H computes an answer A. We store the transcript  $\tau = (Q, Q_1, A_1, \ldots, Q_k, A_k, A)$ .

![](_page_3_Figure_0.jpeg)

Figure 1: Schematic of our Iterated Amplification implementation.

- 2. We train a model H' to predict the decisions made by H in each of these transcripts, i.e. to predict subquestions  $Q_i$  and final answers A.
- 3. We repeatedly sample a question  $Q \sim \mathcal{D}$ , use  $\operatorname{Amplify}^{H'}(X)$  to answer that question, and record the resulting (Q,A) pairs.
...
- This process continues, with X gradually expanding the set of queries it can answer and gradually improving the answers it provides. At each point in training,  $\operatorname{Amplify^H}(X)$  is modestly smarter than X working on its own, and X chases this moving target.
...
subquestion is answered by an independent copy of X trained by  $Amplify^H(X)$ , while in a debate the subquestions are answered by one of the debaters (who are trained to defend a particular answer to the top-level question).
...
- Given a permutation  $\sigma: \{1, \ldots, 64\} \to \{1, \ldots, 64\}$ , compute  $\sigma^k(x)$  for k up to 64.
- Given a function  $f:\{1,\ldots,8\}^2 \to \{1,\ldots,8\}$  and a sequence of 64 assignments of the form x:=3 or x:=f(y,z), evaluate a particular variable.
- Given a function  $f:\{0,1\}^6 \to \{-1,0,1\}$ , answer questions of the form "What is the sum of f(x) over all x matching the wildcard expression 0\*\*1\*\*?"
...
In addition to requiring modestly more training steps, training with amplification requires about twice as much computation per question, because we need to actually generate the training targets by

![](_page_6_Figure_0.jpeg)

Figure 2: Comparison between Iterated Amplification (orange) and supervised learning from the ground truth data (blue). To better illustrate the dynamics of training, in Appendix A we compare the performance of X and  $\operatorname{Amplify}^{H'}(X)$  separately for questions requiring different recursion depths.

running  $\operatorname{Amplify}^{H'}(X)$ .<sup>3</sup> We trained in the same wall-clock time by generating data on a parallel worker.
...
- In our experiments X is trained by supervised learning from  $\operatorname{Amplify^H}(X)$ . In many important applications we suspect that we would learn a reward function from  $\operatorname{Amplify^H}(X)$  and then train X to maximize that reward function.
- In order for Iterated Amplification to succeed, the question distribution  $\mathcal{D}$  needs to be broad enough to cover not only the questions we care about, but also all of the subquestions asked during the computation of  $\operatorname{Amplify}^H(X)$ . The distribution also determines how the model will allocate its capacity, and so must be carefully chosen. In our experiments, we started with a distribution  $\mathcal{D}$  that could be used directly. In a more realistic setting, we might start with some distribution  $\mathcal{D}_0$  of questions that have intrinsic interest, and it would be the system designer's responsibility to construct  $\mathcal{D}$  appropriately.
...
<sup>&</sup>lt;sup>3</sup>Running Amplify<sup>H'</sup>(X) requires calling both X and H' between 3 and 10 times. We train on each (Q, A) pair about 10 times before removing it from the dataset. So the time required to generate a (Q, A) pair is comparable to the total time spent training on it, resulting in roughly twice the total computation per question.
...
- We don't require that the collaboration be efficient, it just needs to help *at all*. If ten agents working together perform "10% better" than a single agent on its own, then  $\operatorname{Amplify}^H(X)$  still provides a useful training signal that we can use to improve X.
- The copies of X don't need to run in parallel—each can start after the previous one has finished its task. Many tasks may be inherently difficult to parallelize, which is an obstacle for human collaboration but is fine for Amplify<sup>H</sup>(X).
...
Table 2: Total number of queries to the decomposition oracle H for each task. These experiments involve algorithmic tasks where these decompositions are straightforward; the sample complexity will likely be much larger for more realistic tasks.

| Task         | Permutation powering | Expression evaluation | Union find | Shortest path | Wildcard search |
|--------------|----------------------|-----------------------|------------|---------------|-----------------|
...
Figure 3 illustrates how X "chases" the moving target  $\operatorname{Amplify}^{H'}(X)$  over the course of training, separating out performance for separate recursion depths d.

X always has lower accuracy than  $\operatorname{Amplify^{H'}}(X)$ , because it is being trained to imitate it.  $\operatorname{Amplify^{H'}}(X)$  for tasks at depth d+1 has slightly lower accuracy than X does for tasks at depth d, because  $\operatorname{Amplify^{H'}}(X)$  answering a question correctly at depth d+1 requires X to answer several questions correctly at depth d.
...
Figure 3: Performance at the permutation powering task, for instances requiring different recursion depths. The green curves are the accuracy of  $\operatorname{Amplify}^{H'}(X)$ ; the red curves are the accuracy of X, The curves further to the right are higher depths, i.e. larger powers of the permutation. For example, the leftmost green curve is the performance of  $\operatorname{Amplify}^{H'}(X)$  at squaring or cubing permutations, which quickly converges to 1 because this task can be accomplished without X's help. The rightmost red curve is the performance of X on powers between 32 and 63.
...
We emphasize that this need not be an *efficient* decomposition in order to be suitable for Iterated Amplification—answers to the subquestion just need to help  $at\ all$  on the original task. As long as that's true, we can use a final copy of X to answer the question in light of these subanswers. This

will outperform a copy of X who didn't get to see the subanswers, and can hence provide a useful training signal for X to improve.
...
An overview of our five tasks is given in Table 3. A task describes a context, a family of questions, a decomposition strategy, and a set of *primitive questions*. The primitive questions are the mechanism by which  $\operatorname{Amplify}^H(X)$  is able to learn about the context. When H asks a primitive question, it immediately receives the correct answer rather than being answered by X.
...
<sup>&</sup>lt;sup>4</sup>We exclude answers of "?", since these are easy to correctly predict, but don't indicate that the algorithm has mastered the task.

Table 3: The tasks on which we trained our algorithm. All tasks are paremeterized by a difficulty N that is increased from 8 to 64 over the course of training. The decompositions marked with \* are significant simplifications of the full decompositions, which are described in Appendix C.3.

| significant simplifications of the full decompositions, which are described in Appendix C.3. |                                                                                                                                                             |  |  |  |  |  |
|----------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|--|--|--|--|--|
|                                                                                              | Permutation powering                                                                                                                                        |  |  |  |  |  |
| Context                                                                                      | A permutation $\sigma$ of $N$ elements.                                                                                                                     |  |  |  |  |  |
| Questions                                                                                    | What is $\sigma^k(x)$ ? (for $2 \le k < 64$ )                                                                                                               |  |  |  |  |  |
| Primitive questions                                                                          | What is $\sigma(x)$ ?                                                                                                                                       |  |  |  |  |  |
| Decomposition                                                                                | $\sigma^{2k}(x) = \sigma^k (\sigma^k(x))$ $\sigma^{2k+1}(x) = \sigma(\sigma^k (\sigma^k(x)))$                                                               |  |  |  |  |  |
|                                                                                              | Sequential assignments                                                                                                                                      |  |  |  |  |  |
| Context                                                                                      | A function $f:\{1,2,\ldots,8\} \times \{1,2,\ldots,8\} \rightarrow \{1,2,\ldots 8\}$<br>A sequence of $N$ definitions of the form $x:=f(y,z)$ or $x:=7$     |  |  |  |  |  |
| Questions                                                                                    | What is the value of $x$ ?                                                                                                                                  |  |  |  |  |  |
| Primitive questions                                                                          | What is the definition of $x$ ?<br>What is $f(a,b)$ ?                                                                                                       |  |  |  |  |  |
| Decomposition                                                                                | Look up definition of $x$ .<br>If $x := f(y, z)$ : evaluate $y$ , evaluate $z$ , and look up $f(y, z)$ .                                                    |  |  |  |  |  |
| Union find                                                                                   |                                                                                                                                                             |  |  |  |  |  |
| Context                                                                                      | An undirected forest with $N$ vertices, $\sqrt{N}$ connected components, and one vertex assigned a label in $\{1, \ldots, 8\}$ in each component.           |  |  |  |  |  |
| Questions                                                                                    | What is the unique label in the component containing $x$ ? What is a vertex on the path from $x$ to a labeled vertex? How far is $x$ from a labeled vertex? |  |  |  |  |  |
| Primitive questions                                                                          | What is a random neighbor of $x$ ?  Are $x$ and $y$ connected?  Is $x$ assigned a label, and if so what is it?                                              |  |  |  |  |  |
| Decomposition*                                                                               | What is a vertex on the path from $x$ to a labeled vertex? If $y$ is the result: what is the unique label in the component containing $y$ ?                 |  |  |  |  |  |
|                                                                                              | Wildcard search                                                                                                                                             |  |  |  |  |  |
| Context                                                                                      | A function $f: \{0,1\}^6 \rightarrow \{-1,0,1\}$ with $N$ non-zero values.                                                                                  |  |  |  |  |  |
| Questions                                                                                    | A function $f: \{0,1\}^* \to \{-1,0,1\}$ with N non-zero values.<br>What is $\sum f(x)$ over x matching a wildcard expression (e.g. $0 **1*0$ )?            |  |  |  |  |  |
| Primitive questions                                                                          | What is $f(x)$ ?                                                                                                                                            |  |  |  |  |  |
| Decomposition                                                                                | Fill in 0 for the first $*$ in the wildcard.<br>Fill in 1 for the first $*$ in the wildcard.<br>Add the results                                             |  |  |  |  |  |
| Shortest path                                                                                |                                                                                                                                                             |  |  |  |  |  |
| Context                                                                                      | A directed graph with $2N$ edges and $N$ vertices.                                                                                                          |  |  |  |  |  |
| Questions                                                                                    | What is the distance from $x$ to $y$ ? What is the first vertex on the path from $x$ to $y$ ?                                                               |  |  |  |  |  |
| Primitive questions                                                                          | What is a random neighbor of $x$ ?  Is there an edge from $x$ to $y$ ?                                                                                      |  |  |  |  |  |
| Decomposition*                                                                               | What is the first vertex on the path from $x$ to $y$ ?<br>If $z$ is the result: what is the distance from $z$ to $y$ ?                                      |  |  |  |  |  |
|                                                                                              | ·                                                                                                                                                           |  |  |  |  |  |

- Test if y is adjacent to x. If so, return 1.<sup>5</sup>
...
We process each sequence by embedding each token in a space of dimension  $d_{\rm embed}=100$ , concatenating the embeddings, and then applying a learned linear transformation into a space of dimension  $d_{\rm model}=512$ . Note that there is no position encoding, because we process unordered sets of facts.

We then process a set of sentences by applying a sequence of N identical layers. Each layer implements the transformation  $x \to z$ :

```
y \leftarrow \text{LayerNorm} \, x + \text{Attention} \, x z \leftarrow \text{LayerNorm} \, y + \text{BatchNorm} \, \text{MLP} \, y
```

where Attention is multi-headed attention with 8 heads of size  $d_{\rm model}/8$ , MLP is a two-layer perceptron with hidden unit size  $4d_{\rm model}$  and ReLU activation, LayerNorm is defined as usual, and BatchNorm normalizes across both the batch and the different sentences.

<sup>&</sup>lt;sup>5</sup>The distance from x to x is taken to be the length of the shortest cycle that contains x, rather than 0.
