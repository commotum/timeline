...
<sup>&</sup>lt;sup>4</sup>https://github.com/jzhang38/TinyLlama
...
|-------------------------------------------------------------------|-----|--------|------|----------|------------|------------|-----|------------|------------|------------|--|--|--|
| 1,10001                                                           | L'  | Tokens | 2K   | 4K       | 8K         | 16K        | 2K  | 4K         | 8K         | 16K        |  |  |  |
| Original LMs                                                      |     |        |      |          |            |            |     |            |            |            |  |  |  |
| RoPE                                                              | -   | -      | 14.5 | 491.4    | 488.5      | 599.5      | 3.5 | 303.0      | 432.1      | 759.5      |  |  |  |
| NoPE                                                              | _   | _      | 14.6 | 326.9    | $> 10^{3}$ | $> 10^{3}$ | 3.5 | 117.4      | $> 10^{3}$ | $> 10^{3}$ |  |  |  |
...
|----------------------|-----------------|----------|----------------|---------------|--|--|
| 1120401              | PG19 Proof-pile |          | I district (†) | ()            |  |  |
...
**Searching scales.** We approach the search for optimal head-based scales  $\lambda^{(h)}$  by parameter-efficient fine-tuning. We use a large learning rate (LR, =0.05 or =0.1) for fine-tuning, as  $\lambda$  spans a wide range, (e.g.,  $[\frac{1}{\sqrt{d}}, \frac{3}{\sqrt{d}}]$ , shown in Figure 5). The fine-tuning data comes from the pretraining dataset (Slimpajama (Soboleva et al., 2023) and Starcoderdata (Li et al., 2023)) with a different data fetching seed from the pretraining. We set the batch size to 8 and set the optimizer to the AdamW ( $\beta_1 = 0.9$ ,
