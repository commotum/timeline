...
Scott Reed<sup>\*,†</sup>, Konrad Żołna<sup>\*</sup>, Emilio Parisotto<sup>\*</sup>, Sergio Gómez Colmenarejo<sup>†</sup>, Alexander Novikov, Gabriel Barth-Maron, Mai Giménez, Yury Sulsky, Jackie Kay, Jost Tobias Springenberg, Tom Eccles, Jake Bruce, Ali Razavi, Ashley Edwards, Nicolas Heess, Yutian Chen, Raia Hadsell, Oriol Vinyals, Mahyar Bordbar and Nando de Freitas<sup>†</sup>

\*Equal contributions, †Equal senior contributions, All authors are affiliated with DeepMind

reedscot@deepmind.com
...
- Images are first transformed into sequences of non-overlapping  $16 \times 16$  patches in raster order, as done in ViT (Dosovitskiy et al., 2020). Each pixel in the image patches is then normalized between [-1,1] and divided by the square-root of the patch size (i.e.  $\sqrt{16} = 4$ ).
...
After tokenization and sequencing, we apply a parameterized embedding function  $f(\cdot; \theta_e)$  to each token (i.e. it is applied to both observations and actions) to produce the final model input. To enable efficient learning from our multi-modal input sequence  $s_{1:L}$  the embedding function performs different operations depending on the modality the token stems from:
...
$$\mathcal{L}(\theta, \mathcal{B}) = -\sum_{b=1}^{|\mathcal{B}|} \sum_{l=1}^{L} m(b, l) \log p_{\theta} \left( s_{l}^{(b)} | s_{1}^{(b)}, \dots, s_{l-1}^{(b)} \right)$$
 (2)
...
|-------------------------------|-------|----------|---------|--------|
| Control chynolinicht          | Lasks | Lpisodes | Tokens  | Weight |
...
where N it the total number of collected episodes for the task, W is the window size, and  $R_i$  is the total return for episode i. To obtain accurate estimates, in practice, we set W to be 10% of the total data amount or a minimum of 1000 episodes (i.e.  $W = \min(1000, 0.1 \times N)$ ).
...
<sup>&</sup>lt;sup>1</sup>The full list of games: Assault, Atlantis, Bank heist, Battle zone, Bowling, Crazy climber, Defender, Fishing derby, Gopher, Hero, Ice hockey, Jamesbond, Kangaroo, Kung fu master, Name this game, Pong, Road runner, Robotank, Tennis, Time pilot, Up n down, Wizard of wor, Zaxxon.

<sup>&</sup>lt;sup>2</sup>The only three tasks below 80% success rate are GoToImpUnlock (59%), Unlock (74%), and BossLevel (75%).
...
|-----------------------------------|-------------------|---------|---------------------|------------------------|-----------------------|---------|
| GATO<br>BC-IMP (LEE ET AL., 2021) | <b>24.5</b> % 23% |         | <b>50.5</b> % 39.3% | 76.5%<br><b>77.5</b> % | $\frac{66.5\%}{66\%}$ |         |
...
| AGENT                             | GROUP 1 | GROUP 2        | GROUP 3 | GROUP 4 | GROUP 5 | Average |
|-----------------------------------|---------|----------------|---------|---------|---------|---------|
| GATO<br>BC-IMP (LEE ET AL., 2021) |         | 57.6%<br>60.8% |         |         |         |         |
...
|----------------------------------------------------------------|-------------------------------------|------|-----------|
| TRANSFORMER BLOCKS ATTENTION HEADS LAYER WIDTH                 | 24                                  | 12   | 8         |
|                                                                | 16                                  | 12   | 24        |
|                                                                | 2048                                | 1536 | 768       |
| FEEDFORWARD HIDDEN SIZE                                        | 8192                                | 6144 | 3072 $32$ |
| KEY/VALUE SIZE                                                 | 128                                 | 128  |           |
| SHARED EMBEDDING<br>LAYER NORMALIZATION<br>ACTIVATION FUNCTION | TRUE PRE-NORM GEGLU (SHAZEER, 2020) |      |           |
...
Optimizer: For all models we use the AdamW (Loshchilov & Hutter, 2017) optimizer with a linear warm-up and cosine schedule decay. The linear warmup lasts for 15,000 steps, starting from a learning rate of 1e-7 and ending at a different maximum learning rate depending on the model (see Table 6). This learning rate is then cosine decayed by a factor 10x over 1,000,000 steps. The AdamW optimizer has parameters  $\beta_1 = 0.9$ ,  $\beta_2 = 0.95$  and  $\epsilon = 1e-8$ . We use a batch size of 512 and a sequence length of 1024 tokens for all models.
...
|---------------------------------------------|---------------|--------------|-----|
| MAXIMUM LEARNING RATE MINIMUM LEARNING RATE |               | 2E-4<br>2E-5 |     |

## E Fine-tuning Setup

**Optimizer:** For all models we use the Adam (Kingma & Ba, 2014) optimizer with a constant learning rate of 1e-5. The Adam optimizer has parameters  $\beta_1 = 0.9$ ,  $\beta_2 = 0.95$  and  $\epsilon = 1e-8$ . We use a batch size of 64 and a sequence length of 1024 tokens for all models. We train for 10,000 gradient steps.
...
We collect two separate sets of Atari environments. The first (that we refer to as ALE Atari) consists of 51 canonical games from the Arcade Learning Environment (Bellemare et al., 2013). The second (that we refer to as ALE Atari Extended) is a set of alternative games<sup>3</sup> with their game mode and difficulty randomly set at the beginning of each episode.
...
<sup>&</sup>lt;sup>3</sup>Basic Math, Breakout, Crossbow, Darkchambers, Entombed, ET, Flag Capture, Human Cannonball, Klax, Laser Gates, Ms. Pac-Man, Solaris, Space War.
...
Additional information on the 18 parent levels (and their relation to the other levels) is presented in details in the NeurIPS Workshop talk A Methodology for RL Environment Research by Daniel Tanis<sup>4</sup>.
...
<sup>&</sup>lt;sup>4</sup>Available at https://neurips.cc/virtual/2021/workshop/21865#wse-detail-22801.
...
<sup>&</sup>lt;sup>5</sup>We used a version from July 23rd 2021, specifically the following version: https://github.com/rlworkgroup/metaworld/commit/a0009ed9a208ff9864a5c1368c04c273bb20dd06.
