...
PROMPT_PATH = "prompt-positional-encoding-sort.md"
OUTPUT_LABEL_FILES = {
    "PE-ROPE": "PE-ROPE.md",
    "PE-LEARNED-ABSOLUTE": "PE-LEARNED-ABSOLUTE.md",
    "PE-SINUSOIDAL-ABSOLUTE": "PE-SINUSOIDAL-ABSOLUTE.md",
    "PE-RELATIVE-POSITION": "PE-RELATIVE-POSITION.md",
    "PE-ALIBI": "PE-ALIBI.md",
    "PE-MIXED": "PE-MIXED.md",
    "PE-NONE-OR-IMPLICIT": "PE-NONE-OR-IMPLICIT.md",
    "PE-INHERITED-DEFAULT": "PE-INHERITED-DEFAULT.md",
    "PE-UNCLEAR": "PE-UNCLEAR.md",
}
CODEX_CLI_CMD = "codex"
DRY_RUN = False
OVERWRITE_MD = False
SORT_MODE = "alpha"
PARALLEL_WORKERS = 4
LOG_PATH = "codex_positional_encoding_sort.log"
...
def build_prompt(
    ...
    file_stem,
    output_folder,
    hint_transformer_yes,
    hint_transformer_no,
    hint_dimension_1d,
    hint_dimension_2dplus,
    hint_task_domains_md,
    hint_task_domains_csv,
):
    prompt = template
    replacements = {
        ...
        "[HINT_TRANSFORMER_YES_MD_ABS_PATH]": maybe_path(hint_transformer_yes),
        "[HINT_TRANSFORMER_NO_MD_ABS_PATH]": maybe_path(hint_transformer_no),
        "[HINT_DIMENSION_1D_MD_ABS_PATH]": maybe_path(hint_dimension_1d),
        "[HINT_DIMENSION_2DPLUS_MD_ABS_PATH]": maybe_path(hint_dimension_2dplus),
        "[HINT_TASK_DOMAINS_MD_ABS_PATH]": maybe_path(hint_task_domains_md),
        "[HINT_TASK_DOMAINS_CSV_ABS_PATH]": maybe_path(hint_task_domains_csv),
    }
...
def make_paper_entry(paper_folder):
    file_stem = os.path.basename(os.path.normpath(paper_folder))
    source_md = os.path.join(paper_folder, f"{file_stem}.md")
    label_paths = {
        label: os.path.abspath(os.path.join(paper_folder, fn))
        for label, fn in OUTPUT_LABEL_FILES.items()
    }
    return {
        "name": file_stem,
        "folder": paper_folder,
        ...
        "hint_transformer_yes": os.path.abspath(os.path.join(paper_folder, "TRANSFORMER-YES.md")),
        "hint_transformer_no": os.path.abspath(os.path.join(paper_folder, "TRANSFORMER-NO.md")),
        "hint_dimension_1d": os.path.abspath(os.path.join(paper_folder, "DIMENSION-1D.md")),
        "hint_dimension_2dplus": os.path.abspath(os.path.join(paper_folder, "DIMENSION-2DPLUS.md")),
...
def list_paper_entries(target_folders, log_path):
    entries = []
    for folder in target_folders:
        ...
        if os.path.isfile(direct_source_md):
            entries.append(make_paper_entry(folder))
            continue
        ...
        for name in os.listdir(folder):
            ...
            if not os.path.isfile(source_md):
                ...
                continue
            entries.append(make_paper_entry(paper_folder))
    return entries
...
def make_temp_md_path(folder):
    while True:
        temp_name = f".PE.tmp.{uuid.uuid4().hex}.md"
...
def parse_decision_from_markdown(md_path):
    ...
    m = re.search(
        r"^\s*Decision\s*:\s*(PE-[A-Z0-9\-]+)\b",
        text,
        flags=re.IGNORECASE | re.MULTILINE,
    )
    if m:
        decision = m.group(1).upper()
        if decision in OUTPUT_LABEL_FILES:
            return decision

    # Fallback broad search
    for label in OUTPUT_LABEL_FILES:
        if re.search(rf"\b{re.escape(label)}\b", text, flags=re.IGNORECASE):
            return label
    return None


def detect_existing_label(label_paths):
    existing = [label for label, path in label_paths.items() if os.path.exists(path)]
    if len(existing) == 0:
        return "NONE", None
    if len(existing) == 1:
        label = existing[0]
        return label, label_paths[label]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run Codex CLI to produce exactly one PE-*.md label file per paper."
    )
    parser.add_argument(
        "--folders",
        nargs="*",
...
def main():
    ...
    state_path = os.path.join(script_dir, ".codex_positional_encoding_sort_state.json")

    log_event(
        log_path,
        "run_start "
        f"folders={resolved_folders} dry_run={dry_run} overwrite={overwrite} sort_mode={sort_mode} workers={workers}",
    )
    ...
    entries = list_paper_entries(resolved_folders, log_path)
    ...
    try:
        ...
        for entry in queue:
            ...
            label_paths = entry["label_paths"]

            log_event(log_path, f"file_start source_md={source_md_abs_path}")

            if not overwrite and source_md_abs_path in completed:
                existing_label, _ = detect_existing_label(label_paths)
                if existing_label in OUTPUT_LABEL_FILES:
                    log_event(log_path, f"skipped_state source_md={source_md_abs_path}")
                    progress.update(file_stem, "skipped")
                    continue

            existing_label, existing_path = detect_existing_label(label_paths)
            if not overwrite:
                if existing_label in OUTPUT_LABEL_FILES:
                    log_event(
                        log_path,
                        f"skipped_existing_output source_md={source_md_abs_path} label={existing_label} path={existing_path}",
                    )
                    if not dry_run:
                    ...
                    progress.update(file_stem, "skipped")
                    continue
                if existing_label == "MULTIPLE":
                    log_event(log_path, f"conflict_multiple_outputs source_md={source_md_abs_path}")
                    progress_write(
                        progress,
                        f"Conflict: multiple PE label files exist for {file_stem}. Re-run with --overwrite.",
                    )
            ...
            if overwrite and not dry_run:
                for path in label_paths.values():
                    remove_existing_output(path, log_path)

            md_work_path = make_temp_md_path(entry["folder"])
            if not dry_run:
                ...
                log_event(log_path, f"md_work_created source_md={source_md_abs_path} md_work={md_work_path}")
            ...
            prompt = build_prompt(
                ...
                file_stem,
                entry["folder"],
                entry["hint_transformer_yes"],
                entry["hint_transformer_no"],
                entry["hint_dimension_1d"],
                entry["hint_dimension_2dplus"],
                entry["hint_task_domains_md"],
                entry["hint_task_domains_csv"],
            )
            log_event(log_path, f"prompt_generated source_md={source_md_abs_path} md_work={md_work_path}")
        ...
        if executor is not None:
            for future in as_completed(futures):
                source_md_abs_path, file_stem, md_work_path, label_paths = futures[future]
                try:
                ...
                if decision not in OUTPUT_LABEL_FILES:
                    log_event(
                        log_path,
                    ...
                    progress_write(
                        progress,
                        f"warning: decision parse failed for {file_stem}; expected one of {sorted(OUTPUT_LABEL_FILES)}",
                    )
                    remove_temp_md(md_work_path, log_path)
                    temp_paths.discard(md_work_path)
                ...
                target_path = label_paths[decision]
                try:
                ...
                log_event(
                    log_path,
                    f"pe_written source_md={source_md_abs_path} decision={decision} md_final={target_path}",
                )
                completed[source_md_abs_path] = {
                    "completed_at": now_iso(),
                    "decision": decision,
