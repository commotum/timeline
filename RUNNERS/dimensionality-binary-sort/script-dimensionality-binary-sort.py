...
PROMPT_PATH = "prompt-dimensionality-binary-sort.md"
ONE_D_FILENAME = "DIMENSION-1D.md"
TWO_D_PLUS_FILENAME = "DIMENSION-2DPLUS.md"
CODEX_CLI_CMD = "codex"
DRY_RUN = False
OVERWRITE_MD = False
SORT_MODE = "alpha"
PARALLEL_WORKERS = 4
LOG_PATH = "codex_dimensionality_binary.log"
...
def build_prompt(
    ...
    replacements = {
        ...
        "[HINT_TRANSFORMER_YES_MD_ABS_PATH]": maybe_path(transformer_yes_md),
        "[HINT_TRANSFORMER_NO_MD_ABS_PATH]": maybe_path(transformer_no_md),
    }
...
def make_paper_entry(paper_folder, one_d_name, two_d_plus_name):
    ...
    return {
        ...
        "output_1d": os.path.abspath(os.path.join(paper_folder, one_d_name)),
        "output_2dplus": os.path.abspath(os.path.join(paper_folder, two_d_plus_name)),
        "hint_task_domains_md": os.path.abspath(os.path.join(paper_folder, "TASK-DOMAINS.md")),
        "hint_task_domains_csv": os.path.abspath(os.path.join(paper_folder, "TASK-DOMAINS.csv")),
        "hint_task_model_ratio_md": os.path.abspath(os.path.join(paper_folder, "TASK_MODEL_RATIO.md")),
        "hint_transformer_yes_md": os.path.abspath(os.path.join(paper_folder, "TRANSFORMER-YES.md")),
        "hint_transformer_no_md": os.path.abspath(os.path.join(paper_folder, "TRANSFORMER-NO.md")),
    }


def list_paper_entries(target_folders, one_d_name, two_d_plus_name, log_path):
    entries = []
    for folder in target_folders:
        ...
        if os.path.isfile(direct_source_md):
            entries.append(make_paper_entry(folder, one_d_name, two_d_plus_name))
            continue
        ...
        for name in os.listdir(folder):
            ...
            if not os.path.isfile(source_md):
                ...
                continue
            entries.append(make_paper_entry(paper_folder, one_d_name, two_d_plus_name))
    return entries
...
def make_temp_md_path(folder):
    while True:
        temp_name = f".DIMENSION.tmp.{uuid.uuid4().hex}.md"
...
def parse_decision_from_markdown(md_path):
    ...
    m = re.search(
        r"^\s*Decision\s*:\s*DIMENSION-(1D|2DPLUS)\b",
        text,
        flags=re.IGNORECASE | re.MULTILINE,
    )
    if m:
        return "DIMENSION-" + m.group(1).upper()

    m = re.search(r"\bDIMENSION-(1D|2DPLUS)\b", text, flags=re.IGNORECASE)
    if m:
        return "DIMENSION-" + m.group(1).upper()
...
def detect_existing_binary(output_1d, output_2dplus):
    one_exists = os.path.exists(output_1d)
    plus_exists = os.path.exists(output_2dplus)
    if one_exists and plus_exists:
        return "BOTH", None
    if one_exists:
        return "1D", output_1d
    if plus_exists:
        return "2DPLUS", output_2dplus
    return "NONE", None
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run Codex CLI to produce DIMENSION-1D.md or DIMENSION-2DPLUS.md per paper."
    )
    parser.add_argument(
        "--folders",
        nargs="*",
    ...
    parser.add_argument(
        "--one-d-name",
        default=ONE_D_FILENAME,
        help="1D output markdown filename per paper folder.",
    )
    parser.add_argument(
        "--two-d-plus-name",
        default=TWO_D_PLUS_FILENAME,
        help="2D+ output markdown filename per paper folder.",
    )
    parser.add_argument(
        "--codex-cmd",
...
def main():
    ...
    one_d_name = args.one_d_name or ONE_D_FILENAME
    two_d_plus_name = args.two_d_plus_name or TWO_D_PLUS_FILENAME
    if not one_d_name.lower().endswith(".md"):
        one_d_name += ".md"
    if not two_d_plus_name.lower().endswith(".md"):
        two_d_plus_name += ".md"
    ...
    state_path = os.path.join(script_dir, ".codex_dimensionality_binary_state.json")

    log_event(
        log_path,
        "run_start "
        f"folders={resolved_folders} dry_run={dry_run} overwrite={overwrite} "
        f"sort_mode={sort_mode} workers={workers} one_d_name={one_d_name} two_d_plus_name={two_d_plus_name}",
    )
    ...
    entries = list_paper_entries(resolved_folders, one_d_name, two_d_plus_name, log_path)
    ...
    try:
        ...
        for entry in queue:
            ...
            output_1d_abs_path = entry["output_1d"]
            output_2dplus_abs_path = entry["output_2dplus"]

            log_event(
                log_path,
                "file_start "
                f"source_md={source_md_abs_path} output_1d={output_1d_abs_path} output_2dplus={output_2dplus_abs_path}",
            )

            if not overwrite and source_md_abs_path in completed:
                status, _ = detect_existing_binary(output_1d_abs_path, output_2dplus_abs_path)
                if status in ("1D", "2DPLUS"):
                    log_event(log_path, f"skipped_state source_md={source_md_abs_path}")
                    progress.update(file_stem, "skipped")
                    continue

            status, existing_path = detect_existing_binary(output_1d_abs_path, output_2dplus_abs_path)
            if not overwrite:
                if status in ("1D", "2DPLUS"):
                    log_event(
                        log_path,
                    ...
                    if not dry_run:
                        decision = "DIMENSION-1D" if status == "1D" else "DIMENSION-2DPLUS"
                    ...
                    continue
                if status == "BOTH":
                    log_event(log_path, f"conflict_both_outputs source_md={source_md_abs_path}")
                    progress_write(
                        progress,
                        f"Conflict: both 1D and 2D+ files exist for {file_stem}. Re-run with --overwrite.",
                    )
                    progress.update(file_stem, "conflict")
                    continue

            if overwrite and not dry_run:
                remove_existing_output(output_1d_abs_path, log_path)
                remove_existing_output(output_2dplus_abs_path, log_path)

            md_work_path = make_temp_md_path(entry_folder)
            if not dry_run:
                if not ensure_md_file(md_work_path, dry_run, log_path):
                    log_event(log_path, f"skipped_md_prepare_failed source_md={source_md_abs_path}")
            ...
            prompt = build_prompt(
                ...
                entry["hint_transformer_yes_md"],
                entry["hint_transformer_no_md"],
            )
            log_event(
                log_path,
        ...
        if executor is not None:
            for future in as_completed(futures):
                ...
                try:
                    ...
                    log_event(log_path, f"codex_failure source_md={source_md_abs_path} error={exc}")
                    remove_temp_md(md_work_path, log_path)
                    temp_paths.discard(md_work_path)
                    progress.update(file_stem, "failed")
                    continue
                ...
                if not success:
                    log_event(log_path, f"codex_failure source_md={source_md_abs_path} error={error}")
                    remove_temp_md(md_work_path, log_path)
                    temp_paths.discard(md_work_path)
                    if error == "codex_missing":
                        progress_write(progress, "Codex CLI not found. Paste this prompt into Codex manually:")
                        ...
                        log_event(log_path, f"manual_prompt_printed source_md={source_md_abs_path}")
                    status = "failed"
                ...
                if not md_has_content(md_work_path):
                    ...
                    progress_write(progress, f"warning: output file is empty for {file_stem} (see log)")
                    remove_temp_md(md_work_path, log_path)
                    temp_paths.discard(md_work_path)
                ...
                if decision not in ("DIMENSION-1D", "DIMENSION-2DPLUS"):
                    log_event(
                        log_path,
                    ...
                    progress_write(
                        progress,
                        f"warning: decision parse failed for {file_stem}; expected 'Decision: DIMENSION-1D|DIMENSION-2DPLUS'",
                    )
                    remove_temp_md(md_work_path, log_path)
                    temp_paths.discard(md_work_path)
                ...
                target_path = output_1d_abs_path if decision == "DIMENSION-1D" else output_2dplus_abs_path
                other_path = output_2dplus_abs_path if decision == "DIMENSION-1D" else output_1d_abs_path

                remove_existing_output(other_path, log_path)
                try:
                    os.replace(md_work_path, target_path)
                except Exception as exc:
                    log_event(log_path, f"md_replace_failed source_md={source_md_abs_path} error={exc}")
                    remove_temp_md(md_work_path, log_path)
                    temp_paths.discard(md_work_path)
                    progress.update(file_stem, "failed")
                    continue
